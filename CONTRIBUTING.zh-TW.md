...
- **單跑一次確認後才可信。** 有固定 5 秒 sentinel 預算的真實行程 spawn 測試——`api::handlers::instance::tests::{handle_spawn_*, set_model_*}`、`transport::{codex_app_server, opencode_server}` 的 socket 探針、以及 `attached_path_mcp_invariants::bridge_tools_list_no_silent_empty_when_daemon_unreachable`。在這台共用主機上以 16 路平行整套跑時，它們會差幾百毫秒超過預算（一個下午兩次各紅 8 個，5.2–5.6 秒），單跑則不到一秒就過；活的 daemon 不是變因。這裡的紅只有在 `cargo nextest run --features tray -E 'test(<name>)'` 單跑再次失敗時才算證據。其中恰好三條——`transport::codex_app_server::tests::{codex_app_server_output_does_not_escape_to_parent_terminal, managed_app_server_launch_carries_config_before_subcommand_3402}` 與 `transport::opencode_server::teardown_tests::managed_teardown_kills_isolated_term_ignoring_server`——還多一個條件：不能是在同一個 worktree 裡一次被取消或 fail-fast 中止的 nextest run 之後立刻跑（`scripts/preflight.sh` 就是 fail-fast；別的 worktree 的中止影響不到你）。中止會把 daemon 開機測試的 `agend-terminal start --foreground` 行程遺留成孤兒，孤兒還年輕時這三條單跑在任何負載下都會紅，之後自行恢復：唯一乾淨的一輪實測 ≤ 8 分鐘（479 秒），但取樣間隔退化到 8–13 分鐘，所以這個上界誠實但寬鬆；殺掉孤兒會立刻恢復。只是觀察、尚未解釋：超過約 25 分鐘的孤兒就不再擋。計入這類紅之前，先用 `pgrep -f 'agend-terminal start --foreground'` 確認沒有來自你 worktree 的孤兒——沒有孤兒卻仍然紅的，**不在**這條免責範圍內（opencode teardown 這條在另一種主機狀態下就這樣紅過，那是真的紅，t-…-108）。中止後的第一次單跑永遠不是證據。
...
- **永遠不是證據。** `cargo test --bin agend-terminal <filter>`（永遠不會建置 `tests/`，對任何住在那裡的 filter 都印出 `ok` 但零測試）；任何在 binary 的 unit tests 裡就停下來的 fail-fast run——`scripts/preflight.sh` 是 fail-fast，而純 `cargo test --tests` 是一個 binary 接一個跑、遇到第一個紅的就停——因為那時沒有任何 `tests/*.rs` target 跑過，它們守的每一條 invariant 都沒被檢查（shim 狀況存在時這是結構上必然，不是風險：#3531 上 runner 停在 bin target 的 shim 紅，`tests/file_size_invariant.rs` 從未執行，一條真的 LOC 超標兩輪後才被 CI 抓到）；以及在高負載主機上整套跑 `cargo test --tests`（每個 binary 一個 process、執行緒平行：本機 7367 個 unit tests 紅了 657 個，抽樣單跑全過）。
