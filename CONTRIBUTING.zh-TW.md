...
- **單跑一次確認後才可信。** 有固定 5 秒 sentinel 預算的真實行程 spawn 測試——`api::handlers::instance::tests::{handle_spawn_*, set_model_*}`、`transport::{codex_app_server, opencode_server}` 的 socket 探針、以及 `attached_path_mcp_invariants::bridge_tools_list_no_silent_empty_when_daemon_unreachable`。在這台共用主機上以 16 路平行整套跑時，它們會差幾百毫秒超過預算（一個下午兩次各紅 8 個，5.2–5.6 秒），單跑則不到一秒就過；活的 daemon 不是變因。這裡的紅只有在 `cargo nextest run --features tray -E 'test(<name>)'` 單跑再次失敗時才算證據。其中恰好三條——`transport::codex_app_server::tests::{codex_app_server_output_does_not_escape_to_parent_terminal, managed_app_server_launch_carries_config_before_subcommand_3402}` 與 `transport::opencode_server::teardown_tests::managed_teardown_kills_isolated_term_ignoring_server`——還多一個條件：不能是在同一個 worktree 裡一次被取消或 fail-fast 中止的 nextest run 之後立刻跑（`scripts/preflight.sh` 的 nextest 步驟是 fail-fast——它沒帶 `--no-fail-fast`——腳本本身則會把其餘步驟跑完、最後才以 1 結束；別的 worktree 的中止影響不到你）。中止會把 daemon 開機測試的 `agend-terminal start --foreground` 行程遺留成孤兒，孤兒還年輕時這三條單跑在任何負載下都會紅，之後自行恢復：唯一乾淨的一輪實測 ≤ 8 分鐘（479 秒），但取樣間隔退化到 8–13 分鐘，所以這個上界誠實但寬鬆；殺掉孤兒會立刻恢復。只是觀察、尚未解釋：超過約 25 分鐘的孤兒就不再擋。計入這類紅之前，先在你的 worktree 根目錄用 `pgrep -fl "$PWD/target/debug/agend-terminal start --foreground"` 找孤兒——路徑就是 worktree 的範圍：每個測試 daemon 跑的都是自己 worktree 的 debug binary，所以別的 worktree 的孤兒和活的 daemon（`target/release` 的 binary）永遠比不中。PPID 為 1 的命中就是孤兒；`ps -o pid,ppid,lstart,command -p <pid>` 同時顯示這一點和它的年齡。路徑不能省：不帶路徑的 `pgrep -f 'agend-terminal start --foreground'` 只在 daemon 的後代 shell 裡看不到活的 daemon（`pgrep` 預設排除自己的祖先），換到任何別的 shell——operator 自己的終端機、daemon 重啟前留下的 session——它就會列出來，這時照「殺掉」做等於把全隊 daemon 殺掉。沒有孤兒卻仍然紅的，**不在**這條免責範圍內（opencode teardown 這條在另一種主機狀態下就這樣紅過，那是真的紅，t-…-108）。中止之後，只有在那條帶路徑的 `pgrep` 顯示沒有孤兒時，第一次單跑才算證據；還有孤兒就只殺它列出的那些 PID、或等待，再跑一次。
