...
ran: `cargo test --locked --bin agend-terminal` at the pre-final terminology-only fixture head → 7,491 passed, 9 failed, 11 ignored. The nine failures were in API, daemon, dispatch-tracking, binding-state, and checkout-submodule fixtures; none were in `schedule_jobs::runtime`. They were not reproduced in isolated serial runs, so aggregate causation remains unproven. The final correction is covered by the post-correction 14-test runtime suite above. This result is reported as suite red, not waived.
