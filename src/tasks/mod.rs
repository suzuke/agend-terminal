...
/// Return all tasks as typed structs from the authoritative catalog view.
...
/// honoured by the catalog fold as before.
pub fn list_all(home: &Path) -> Vec<Task> {
    list_all_at(home, home)
}

/// #2117 P1: list a specific board's tasks (catalog view + in-memory dep
...
pub fn migrate_legacy_tasks_json_to_event_log(home: &Path) -> anyhow::Result<MigrationReport> {
    ...
    // `task_events::projected_state()`) no longer touches it. Idempotent: if the
