...
/// #3279: create a successor and terminate its predecessor in one same-board
/// event-log transaction. Both task-id locks are acquired in lexical order so
/// opposite supersession attempts cannot deadlock. No IPC occurs while held.
fn create_with_supersession(
    home: &Path,
    project: &str,
    ...
    if seqs.is_empty() {
        ...
        let task = read_task_record_at(&board, &existing_id).map(|record| record_to_task(&record));
        return serde_json::json!({
            "id": existing_id,
    ...
    let task = read_task_record_at(&board, successor_id).map(|record| record_to_task(&record));
    drop(id_locks);
...
fn handle_create(home: &Path, emitter: crate::task_events::InstanceName, args: &Value) -> Value {
    ...
    {
        return create_with_supersession(
            home,
            &project,
            &emitter,
            &id,
