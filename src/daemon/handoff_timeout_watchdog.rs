    ...
    G: FnMut(&str, &str) -> usize,
{
        ...
        std::collections::HashSet::new();
    {
        for (path, track) in &tracks {
            ...
            // Deferred tracks: evaluate wake predicate, skip renudge/escalation.
            if track.is_deferred() {
                if track.is_defer_expired(now) {
                    crate::daemon::ci_handoff_track::reactivate_track(
                        home,
                        target,
                        &corr,
                        track.ci_handoff_episode.as_deref().unwrap_or(""),
                    );
...
mod tests {
    ...
    fn seed_handoff_with_episode(
        ...
        let sent_at = (chrono::Utc::now() - chrono::Duration::minutes(age_min)).to_rfc3339();
        let mut msg = crate::inbox::InboxMessage::new_system(
            "system:ci",
            HANDOFF_KIND,
            format!("[ci-ready-for-action] {corr}: CI passed."),
        )
        .with_correlation_id(corr);
        msg.timestamp = sent_at.clone();
        crate::inbox::enqueue(home, target, msg).unwrap();
    ...
    fn deferred_track_suppresses_renudge() {
        ...
        assert!(
            nudged.is_empty(),
            "deferred track must suppress renudge: {nudged:?}"
        );
        std::fs::remove_dir_all(home).ok();
    }

    #[test]
    fn deferred_track_expiry_reactivates() {
        let home = tmp_home("defer-expire");
        write_fleet(&home);
        seed_snapshot(&home, "reviewer", "idle");
        ...
        assert!(
            !nudged.is_empty(),
            "expired deferred track must reactivate and renudge"
        );
        let tracks = crate::daemon::ci_handoff_track::list(&home);
        let track = tracks.iter().find(|(_, t)| t.correlation == "o/r@b");
        assert!(
            track.is_some_and(|(_, t)| !t.is_deferred()),
            "track must be reactivated after expiry"
        );
        std::fs::remove_dir_all(home).ok();
    }

    #[test]
    fn defer_episode_mismatch_refused() {
        let home = tmp_home("defer-cas");
        write_fleet(&home);
        ...
        assert!(
            matches!(
                result,
                crate::daemon::ci_handoff_track::DeferOutcome::EpisodeMismatch
            ),
            "episode mismatch must refuse defer"
        );
        std::fs::remove_dir_all(home).ok();
    }

    #[test]
    fn defer_survives_restart() {
        let home = tmp_home("defer-restart");
        write_fleet(&home);
        seed_handoff_with_episode(&home, "reviewer", "o/r@b", 3, "ep-persist");
        ...
        assert!(
            track.is_some_and(|(_, t)| t.is_deferred()),
            "deferred state must persist to disk and survive re-read"
        );
        std::fs::remove_dir_all(home).ok();
    }

    #[test]
    fn defer_via_mcp_entry() {
    ...
    #[test]
    fn status_exposes_deferred_state() {
        let home = tmp_home("defer-status");
        write_fleet(&home);
        seed_handoff_with_episode(&home, "reviewer", "o/r@b", 3, "ep-status");
        ...
        let status = crate::mcp::handlers::dispatch::dispatch_ci(&ctx);
        let handoffs = status["pending_handoffs"]
            .as_array()
            .expect("pending_handoffs array");
        let h = handoffs.iter().find(|h| h["correlation"] == "o/r@b");
        assert!(h.is_some(), "deferred handoff must appear in status");
        let h = h.unwrap();
    ...
    /// #2870 RED: a feature-class CI handoff sidecar must survive ordinary inbox
    /// drains and remain available for re-nudge / escalation by the watchdog.
    /// Current production wrongly deletes the sidecar: the second drain implicitly
    /// marks the row Processed, and `reconcile_processed` (called from the watchdog
    /// tick) deletes the track — killing re-nudge and escalation.
    ///
    /// Episode 1 (RED): feature ci-ready → drain → drain → watchdog at +31s past
    /// grace → assert sidecar survives and re-nudge / escalation still fire.
    ///
    /// Episode 2 (positive control): fresh feature ci-ready → real `ack_handoff` →
    /// sidecar correctly resolved → recreate sidecar (simulating crash before the
    /// delete persisted) → `reconcile_processed` cleans it up → watch preserved.
    #[test]
    fn feature_ci_drain_must_not_lose_renudge_or_escalation_2870() {
        let home = tmp_home("2870-feature-drain");
        write_fleet(&home);
        seed_snapshot(&home, "reviewer", "idle");

        let base = chrono::Utc::now() - chrono::Duration::minutes(15);
        let sent_at = base.to_rfc3339();
        let episode_1 = crate::daemon::ci_handoff_track::new_episode();
        let corr = "owner/repo@feat/2870";

        // ── emit feature ci-ready row + sidecar ──
        let mut msg = crate::inbox::InboxMessage::new_system(
            "system:ci",
            HANDOFF_KIND,
            format!("[ci-ready-for-action] {corr}: CI passed, your turn."),
        ...
        .with_correlation_id(corr);
        msg.timestamp = sent_at.clone();
        msg.ci_handoff_episode = Some(episode_1.clone());
        msg.ci_handoff_class = Some(crate::inbox::CiHandoffClass::Feature);
        crate::inbox::enqueue(&home, "reviewer", msg).unwrap();
        assert!(
            crate::daemon::ci_handoff_track::record_with_identity(
                &home,
                "reviewer",
            ...
            "precondition: track must be recorded"
        );

        // precondition: one track exists
        assert_eq!(
            crate::daemon::ci_handoff_track::list(&home).len(),
            1,
            "precondition: exactly one feature ci-handoff track"
        );

        // ── first drain: row transitions unread → delivering ──
        let d1 = crate::inbox::drain(&home, "reviewer");
        assert!(
            d1.iter().any(|m| m.kind.as_deref() == Some(HANDOFF_KIND)
                && m.ci_handoff_episode.as_deref() == Some(&episode_1)),
        ...
        assert_eq!(
            crate::daemon::ci_handoff_track::list(&home).len(),
            1,
            "track must survive first drain (delivering state)"
        );

        // ── second drain WITHOUT ack_handoff: implicit ack ──
        let d2 = crate::inbox::drain(&home, "reviewer");
        assert!(
            d2.iter()
                .all(|m| m.ci_handoff_episode.as_deref() != Some(&episode_1)),
            "second drain must not re-deliver the already-delivering row"
        );

        // track must survive second drain (the agent did not ack_handoff)
        assert_eq!(
            crate::daemon::ci_handoff_track::list(&home).len(),
            1,
            "#2870 precondition: track survives second drain (no protected settlement for feature class)"
        );

        // ── watchdog at +31s past sent_at → reconcile_processed fires ──
        // sent_at was 15 min ago so we are well past TRACK_RECONCILE_GRACE (30s).
        let now_31s = base + chrono::Duration::seconds(31);
        let mut esc_31 = HashMap::new();
        let mut ren_31 = HashMap::new();
        run_watchdog(&home, &now_31s, &mut esc_31, &mut ren_31);

        // RED: the track must survive reconcile_processed — the agent never
        // called ack_handoff, so the sidecar must remain for re-nudge/escalation.
        // Current production DELETES it here (the implicit second-drain ack made
        // the row Processed, and reconcile_processed sees that as a crash-recovery
        // signal).
        assert_eq!(
            crate::daemon::ci_handoff_track::list(&home).len(),
            1,
        ...
        let now_2m = base + chrono::Duration::minutes(2) + chrono::Duration::seconds(1);
        let mut esc_2m = HashMap::new();
        let mut ren_2m = HashMap::new();
        let nudged = run_watchdog(&home, &now_2m, &mut esc_2m, &mut ren_2m);
        assert!(
            nudged.iter().any(|t| t == "reviewer"),
        ...
        let now_10m = base + chrono::Duration::minutes(10) + chrono::Duration::seconds(1);
        let mut esc_10m = HashMap::new();
        let mut ren_10m = HashMap::new();
        run_watchdog(&home, &now_10m, &mut esc_10m, &mut ren_10m);
        let lead_msgs = crate::inbox::drain(&home, "lead");
        assert!(
            lead_msgs.iter().any(|m| m.text.contains("handoff_timeout_watchdog")
                || m.text.contains("ci-ready")),
            "#2870: watchdog must escalate to lead after ordinary drain — \
             the feature ci-handoff track must not have been silently deleted"
        );

        // ── Episode 2: positive control — real ack_handoff settles correctly ──
        let episode_2 = crate::daemon::ci_handoff_track::new_episode();
        let sent_at_2 = (base + chrono::Duration::minutes(1)).to_rfc3339();

        let mut msg2 = crate::inbox::InboxMessage::new_system(
            "system:ci",
            HANDOFF_KIND,
            format!("[ci-ready-for-action] {corr}: CI passed (ep2), your turn."),
        )
        .with_correlation_id(corr);
        msg2.timestamp = sent_at_2.clone();
        msg2.ci_handoff_episode = Some(episode_2.clone());
        msg2.ci_handoff_class = Some(crate::inbox::CiHandoffClass::Feature);
        crate::inbox::enqueue(&home, "reviewer", msg2).unwrap();
        ...
        // drain once to transition to delivering (ack_handoff needs the row)
        let _d3 = crate::inbox::drain(&home, "reviewer");
        ...
        assert_eq!(
            ...
            Some(true),
            "ack_handoff must succeed for episode 2: {ack_result}"
        );
        assert_eq!(
            ack_result["watch_preserved"].as_bool(),
            Some(true),
            "ack_handoff must preserve watch subscription: {ack_result}"
        );

        // After ack_handoff, the track is resolved (deleted)
        let remaining_ep2: Vec<_> = crate::daemon::ci_handoff_track::list(&home)
            .into_iter()
            .filter(|(_, t)| t.ci_handoff_episode.as_deref() == Some(&episode_2))
            .collect();
        assert!(
            remaining_ep2.is_empty(),
            "ack_handoff must resolve the episode 2 track"
        );
        ...
        // The inbox row is already Processed (ack_handoff settled it). The
        // reconciler should detect this and clean up the orphan sidecar.
        let now_reconcile = base + chrono::Duration::minutes(5);
        let reconciled =
            crate::daemon::ci_handoff_track::reconcile_processed(&home, &now_reconcile);
        assert_eq!(
            reconciled, 1,
            "crash-reconciliation must clean up a re-created sidecar \
             whose inbox row is already Processed via ack_handoff"
        );
        let remaining_after_reconcile: Vec<_> = crate::daemon::ci_handoff_track::list(&home)
            .into_iter()
            .filter(|(_, t)| t.ci_handoff_episode.as_deref() == Some(&episode_2))
            .collect();
        assert!(
            remaining_after_reconcile.is_empty(),
            "reconciler must remove the crash-orphan sidecar"
        );
