...
fn test_instructions(home: &Path) -> TestResult {
    ...
    let _ = instructions::generate(home, &test_dir, "claude");
    ...
    let _ = instructions::generate(home, &test_dir, "kiro-cli");
...
fn test_backend(backend: &backend::Backend, home: &Path) -> Vec<TestResult> {
    ...
    // 1. Instructions
    let _ = crate::instructions::generate(home, &test_dir, preset.command);
