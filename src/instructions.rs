...
pub(crate) fn build_instructions_body(
    ...
    content.push_str("- `inbox` — check pending OR query specific. No params = drain pending; returned rows expose durable `delivery_count` + `first_delivered_at`, and redelivery headers expose the count without changing canonical id/text. With `message_id` = describe message status. With `thread_id` = fetch thread messages. `action=ack` with a message_id can settle a previously delivered/reclaimed row only when durable history proves prior delivery; omitted message_id acknowledges only the current in-flight batch. The response distinguishes `acked-after-reclaim` from `never-delivered` and `already-processed`. Fresh session reset requeues unconfirmed rows for successor recovery.\n");
    ...
    content.push_str("- `repo` — actions: `checkout` / `release` / `merge` / cleanup operations\n");
    ...
    // `[AGEND-AUTO]`/`[AGEND-RESUME]`). The context-handoff
...
mod tests {
    ...
    fn handoff_marker_vocabulary_is_unconditional_2282() {
        ...
        // of mode (like `[AGEND-AUTO]`/`[AGEND-RESUME]`), so an
    ...
    #[test]
    fn fleet_fallback_uses_live_tool_contract_and_section_links() {
        ...
        for stale in [
            ...
            assert!(
                !body.contains(stale),
                "fallback instructions still teach stale contract: {stale}"
            );
        }
        ...
        for current in [
            ...
            assert!(
                ...
                "fallback instructions must teach live contract: {current}"
            );
        }
    }
