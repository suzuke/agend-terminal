use crate::agent_ops::{
    list_agents, merge_metadata, save_metadata, save_metadata_batch, validate_branch,
};
...
/// Inner impl of [`spawn_single_instance`] parameterized on the SPAWN RPC.
///
/// The `spawn_fn` indirection exists for the T1/T2 regression tests: they
/// pass a mock that mimics `handle_spawn`'s side effects (the SPAWN-time
/// `register_topic` → `update_instance_field` chain) without standing up
/// a real daemon. Production passes [`crate::api::call`].
fn spawn_single_instance_impl(
    home: &Path,
    instance_name: &str,
    args: &Value,
