...
fn armed_exact_head_count(home: &Path, repo: &str, branch: &str) -> u64 {
...
/// hardened against.
fn unwatch_exact_head(home: &Path, repo: &str, branch: &str, sha: &str, caller: &str) -> Value {
    if !crate::daemon::ci_watch::is_full_commit_sha(sha) {
        return json!({
            "error": format!("exact-head unwatch requires a FULL immutable commit SHA (40- or 64-hex); got {sha:?}"),
    ...
    if !caller.is_empty() {
        ...
        if notification_only {
            let task_id = watch["task_id"].as_str().unwrap_or("");
            let Some(receipt) = crate::merge_receipt::find(home, repo, &sha, task_id) else {
                return json!({
                    "error": "notification_only exact-head unwatch requires a matching merge receipt (repo + head_sha + task_id)",
                    "code": "exact_head_unwatch_no_receipt",
                });
            };
            if receipt.task_assignee != caller {
                return json!({
                    "error": format!(
                        "exact-head unwatch: caller '{caller}' is not the task assignee '{}'",
                        receipt.task_assignee
                    ),
                    "code": "exact_head_unwatch_unauthorized",
                });
            }
            // Deliberately NO current-binding check here (unlike the arm). The
            // durable authority to drop one's OWN subscription is the exact
            // repo+SHA+task receipt plus `receipt.task_assignee == caller`; by
            // disarm time a legitimate assignee has usually moved on and is bound
            // to a LATER task, and that unrelated binding proves nothing about
            // this watch — gating on it would block exactly the legitimate case.
            // NOT a full-disarm authority. A notification_only assignee owns its
            // OWN subscription only: it removes itself and leaves every other
            // subscriber (and any continuation) intact. Full disarm stays with
            // the operator / privileged orchestrator arm below.
            let mut subscribers = crate::daemon::ci_watch::parse_subscribers(&watch);
            ...
            let authorized = !targets.is_empty()
                && targets
                    .iter()
            ...
            if !authorized {
                return json!({
                    "error": format!(
                        "'{caller}' may not disarm this exact-head watch — only the operator or the orchestrator of every `next_after_ci` target may"
                    ),
                    "code": "exact_head_unwatch_unauthorized",
                });
            }
...
pub(crate) fn handle_unwatch_ci(home: &Path, args: &Value, instance_name: &str) -> Value {
    ...
    if let Some(sha) = args["head_sha"].as_str().filter(|s| !s.is_empty()) {
        return unwatch_exact_head(home, repo, branch, sha, instance_name);
    }
