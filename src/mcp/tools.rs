...
pub(crate) fn def_send() -> Value {
    json!({"name": "send", "description": "Send a message to another instance or broadcast to multiple. Replaces send_to_instance/delegate_task/report_result/request_information/broadcast. Sprint 58 Wave 4 PR-1: kind=task dispatches MUST include task_id (call task action=create first to obtain a 't-...' id).",
        "inputSchema": {"type": "object", "properties": {
            ...
            "review_assignment": {"type": "boolean", "description": "t-…-17: mark this dispatch a DURABLE reviewer assignment (fail-closed). Requires an explicit non-empty `task_id`, non-empty `branch`, and a nonzero `pr_number` (the generation identity). Authority = the SOLE current orchestrator of the exactly-one team whose `source_repo` canonicalizes to the dispatch repo (no operator-allow); the reviewer target must differ from `review_author`. A non-marker dispatch is unaffected."},
