...
fn fugu_home_config_needs_rewrite(body: &str) -> bool {
    let Some(provider) = find_sakana_provider(body) else {
        return true;
    };
    provider
        .name
...
/// if its `config.toml` already exists and satisfies Codex's provider schema it
/// is left untouched. Older agend-generated configs missing provider `name` are
/// repaired in place. Requires an `Available` detection carrying a provider
/// block + catalog path; returns the home path on success.
pub fn ensure_fugu_codex_home(detection: &FuguDetection) -> std::io::Result<PathBuf> {
    let home = fugu_codex_home()
        .ok_or_else(|| std::io::Error::new(std::io::ErrorKind::NotFound, "no home directory"))?;
    ensure_fugu_codex_home_at(detection, home)
}

fn ensure_fugu_codex_home_at(detection: &FuguDetection, home: PathBuf) -> std::io::Result<PathBuf> {
    let config = home.join("config.toml");
    if config.exists() {
        let existing = std::fs::read_to_string(&config)?;
        if !fugu_home_config_needs_rewrite(&existing) {
            return Ok(home);
        }
    }
...
mod tests {
    ...
    fn render_fugu_home_config_selects_fugu_and_provider() {
        ...
        assert!(body.contains("name = 'Sakana API'") || body.contains("name = \"Sakana API\""));
    ...
    #[test]
    fn render_fugu_home_config_quotes_apostrophes_as_valid_toml() {
        let provider = ProviderBlock {
            id: "sakana".to_string(),
            name: Some("Sakana's API".to_string()),
            base_url: Some("https://api.sakana.ai/v1".to_string()),
            env_key: Some("SAKANA_API_KEY=fish_inline".to_string()),
            wire_api: Some("responses".to_string()),
        };
        let body = render_fugu_home_config(
            &provider,
            std::path::Path::new("/Users/x/O'Brien/fugu.json"),
        );
        let parsed: toml::Value = toml::from_str(&body).expect("generated config is valid TOML");
        assert_eq!(
            parsed["model_providers"]["sakana"]["name"].as_str(),
            Some("Sakana's API")
        );
        assert_eq!(
    ...
    #[test]
    fn generated_fugu_config_missing_provider_name_needs_rewrite() {
        let old = "# Auto-generated by agend-terminal (#2441) - isolated Fugu CODEX_HOME.\n\
model = \"fugu\"\nmodel_provider = 'sakana'\n\n[model_providers.sakana]\n\
base_url = 'https://api.sakana.ai/v1'\nenv_key = 'SAKANA_API_KEY=fish_inline'\n\
wire_api = 'responses'\n";
        assert!(fugu_home_config_needs_rewrite(old));

        let fixed = "model = \"fugu\"\nmodel_provider = 'sakana'\n\n[model_providers.sakana]\n\
name = 'Sakana API'\nbase_url = 'https://api.sakana.ai/v1'\nenv_key = 'SAKANA_API_KEY=fish_inline'\n\
wire_api = 'responses'\n";
        assert!(!fugu_home_config_needs_rewrite(fixed));
    }

    #[test]
    fn ensure_fugu_home_errors_without_provider() {
        let home = std::env::temp_dir().join(format!(
            "agend-fugu-home-missing-provider-{}",
            std::process::id()
        ));
        ...
        assert!(ensure_fugu_codex_home_at(&detection, home.clone()).is_err());
        std::fs::remove_dir_all(&home).ok();
    }
}
