...
// ---------------------------------------------------------------------------
//
// #2764 E: the legacy `cleanup_working_dir` / `cleanup_working_dir_proven`
// wrappers are GONE. The only destructive workspace-cleanup entry is
// `workspace_cleanup::full_delete_destructive_phase` (id-anchored, typed
// outcome, exact-id fleet CAS), driven by `full_delete_instance`. The
// canonical 19-entry agend-file scrub list lives at
// `workspace_cleanup::AGEND_FILES`.
...
mod tests {
    ...
    // --- cleanup_working_dir: the legacy wrapper is gone (#2764 E). Its
    // workspace-remove / selective-scrub / 19-entry drift coverage now lives
    // on the real entry: `workspace_cleanup::tests` (phase_* tests drive
    // `full_delete_destructive_phase`, and `AGEND_FILES` is asserted
    // entry-by-entry in `phase_external_scrub_removes_all_agend_files_keeps_
    // user_files`). Metadata-tail coverage lives in
    // `instance_state/lifecycle/tests.rs` (full_delete removes the name-keyed
    // metadata file).
