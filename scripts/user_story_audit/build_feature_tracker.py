...
CLI_ROWS = [
    ...
    ("mode", "Set operator mode", "operator", "set active/away/sleep with optional delegate scope", "agents can respect operator availability and authority boundaries", "mode accepts active, away, or sleep plus delegate/scope values and stores operator authority state.", "CLI", "src/main.rs: Commands::Mode; src/operator_mode.rs", "Set each mode in temp AGEND_HOME and verify list_instances' operator_mode field reflects it (#2548: the MCP `mode get` tool was retired, folded into list_instances).", "P1"),
...
MCP_TOOLS: list[tuple[str, str, str, str, str, str, str]] = [
    ...
    ("replace_instance", "Replace instance fresh", "operator/agent", "replace a stuck instance with a fresh process", "recovery avoids manual teardown", "Retired in #2547 — folded into restart_instance mode=fresh; row kept for audit-history continuity.", "src/mcp/registry.rs; src/mcp/handlers/instance_state/mod.rs"),
    ...
    ("set_display_name", "Set display name", "agent", "change my visible name", "TUI/channel users can identify panes", "Retired in #2547 — merged into set_metadata action=display_name; row kept for audit-history continuity.", "src/mcp/tools.rs; src/mcp/handlers/instance_metadata.rs"),
    ("set_description", "Set description", "agent", "set a short visible description", "the operator can see current role/context", "Retired in #2547 — merged into set_metadata action=description; row kept for audit-history continuity.", "src/mcp/tools.rs; src/mcp/handlers/instance_metadata.rs"),
    ...
    ("tui_screenshot", "Capture TUI screenshot", "agent", "capture the current TUI as SVG", "visual layout issues can be reviewed", "Retired in #2548 (zero calls in 20 days, no CLI replacement — the ScreenshotRequest plumbing was fully removed); row kept for audit-history continuity.", "src/mcp/tools.rs; src/mcp/handlers/dispatch.rs"),
    ...
    ("task_sweep_config", "Task auto-close sweep config", "operator", "configure PR-merge task auto-close sweep", "merged PRs can close task records automatically", "Retired in #2547 — moved to `agend-terminal admin task-sweep-config` CLI (zero MCP calls, operator-only setting); row kept for audit-history continuity.", "src/main.rs; src/daemon/task_sweep.rs"),
    ...
    ("ephemeral", "Ephemeral workers", "agent/operator", "spawn/list/reap short-lived workers", "bounded one-shot helper work can run outside managed fleet bookkeeping", "MCP tool retired in #2547 (zero calls); tracking/reap infra retained pending #2548 P1 decision. Row kept for audit-history continuity.", "src/ephemeral_driver.rs; src/ephemeral_tracking.rs"),
    ...
    ("watchdog", "Fleet idle watchdog", "operator/agent", "snooze/resume/status/ack idle alerts", "idle fleet notifications can be controlled", "Retired in #2548 (zero calls in 20 days) — moved to `agend-terminal admin watchdog` CLI; row kept for audit-history continuity.", "src/mcp/tools.rs; src/daemon/idle_watchdog.rs"),
    ("config", "Runtime config", "operator/agent", "get/list mutable daemon configuration (read-only for agents)", "thresholds and UI flags can be inspected without rebuilding", "#2548: set action moved to `agend-terminal admin config-set` CLI (zero MCP calls in 20 days) — validates known keys and writes runtime-config.json.", "src/mcp/tools.rs; src/runtime_config.rs; docs/FEATURE-configuration.md"),
    ...
    ("force_release_worktree", "Emergency worktree release", "operator", "force clean stale daemon-managed worktrees", "dangling leases can be recovered safely", "Retired in #2548 — folded into `release_worktree(force:true)` (same behavior: requires instance/branch, refuses paths outside daemon pool, restricted to worktree owner/team orchestrator per AUDIT2-002); row kept for audit-history continuity.", "src/mcp/tools.rs; src/mcp/handlers/worktree.rs; src/mcp/handlers/force_release/*"),
    ...
    ("gc_dry_run", "Worktree GC preview", "operator", "preview daemon-managed worktree GC candidates", "cleanup can be audited before deletion", "Retired in #2548 (zero calls in 20 days) — moved to `agend-terminal admin gc-dry-run` CLI; row kept for audit-history continuity.", "src/mcp/tools.rs; src/worktree_pool/gc.rs"),
    ("tokens", "Token/cost summary", "operator", "summarize estimated Claude/Codex token usage", "fleet cost/context consumption can be monitored", "Retired in #2548 (zero calls in 20 days) — moved to `agend-terminal admin tokens` CLI; row kept for audit-history continuity.", "src/mcp/tools.rs; src/token_cost.rs"),
    ("mode", "Read operator mode", "agent", "read active/away/sleep operator mode", "agents can back off when operator is unavailable", "Retired in #2548 — read side folded into `list_instances`'s `operator_mode` field (mode/delegate_to/delegate_scope); setting stays CLI-only. Row kept for audit-history continuity.", "src/mcp/tools.rs; src/mcp/handlers/instance_queries.rs; src/operator_mode.rs"),
]
...
TEST_RESULTS: dict[str, tuple[str, str, str, str, str]] = {
    ...
    "US-MCP-012": ("Pass", "", "N/A", "Pass", "set_display_name set display metadata; tool retired in #2547 — covered by set_metadata action=display_name."),
    "US-MCP-013": ("Pass", "", "N/A", "Pass", "set_description set instance description; tool retired in #2547 — covered by set_metadata action=description."),
    ...
    "US-MCP-020": ("Pass", "", "N/A", "Pass", "task_sweep_config returned config (after mode active); tool retired in #2547 — moved to admin task-sweep-config CLI."),
    ...
    "US-MCP-025": ("Pass", "", "N/A", "Pass", "ephemeral list returned worker pool state (after mode active); tool retired in #2547 — tracking/reap infra retained pending #2548 P1."),
    "US-MCP-026": ("Pass", "", "N/A", "Pass", "ci status returned empty watch list."),
    "US-MCP-027": ("Pass", "", "N/A", "Pass", "health report set rate_limit reason."),
    "US-MCP-028": ("Pass", "", "N/A", "Pass", "watchdog status returned snooze/ack state; tool retired in #2548 — moved to admin watchdog CLI."),
    ...
    "US-MCP-035": ("Pass", "", "N/A", "Pass", "gc_dry_run listed 0 candidates without mutation; tool retired in #2548 — moved to admin gc-dry-run CLI."),
    "US-MCP-036": ("Pass", "", "N/A", "Pass", "tokens summary returned backend cost summary; tool retired in #2548 — moved to admin tokens CLI."),
    "US-MCP-037": ("Pass", "", "N/A", "Pass", "mode get returned operator mode (read-only); tool retired in #2548 — read side folded into list_instances' operator_mode field."),
    # tools/list surface
...
TEST_RESULTS.update({
    ...
    "US-MCP-009": ("Pass", "", "N/A", "Pass", "replace_instance spawned a fresh instance (spawned=true); tool retired in #2547 — behavior now covered by restart_instance mode=fresh."),
    ...
    "US-MCP-017": ("Pass", "", "N/A", "Pass", "tui_screenshot correctly errors in daemon-only mode (requires TUI mode) — expected behaviour; tool retired in #2548 (zero calls, no CLI replacement) — the ScreenshotRequest plumbing and src/screenshot.rs were removed."),
    ...
    "US-MCP-033": ("Pass", "", "N/A", "Pass", "force_release_worktree returned a structured binding_outcome (already_released) without mutating non-pool paths; tool retired in #2548 — folded into release_worktree(force:true), same behavior."),
})
