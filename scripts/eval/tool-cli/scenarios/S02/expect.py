...
def grade(ctx):
    ...
    rows = {r.get("id"): r for r in inbox(ctx, "probe")}
    tgt_id = seed.get("target", {}).get("id")
    dec_id = seed.get("decoy", {}).get("id")
    tgt, dec = rows.get(tgt_id), rows.get(dec_id)
    if tgt is None or dec is None:
        crit.append(critical("completeness"))
        notes.append("seeded row missing (target=%r decoy=%r)"
                     % (tgt is not None, dec is not None))
        return Verdict(False, crit, notes)
    tgt_acked = bool(tgt.get("read_at"))
    dec_acked = bool(dec.get("read_at"))
    # wrong_target must reflect the agent's INTENT, not daemon semantics: a
    # second plain `inbox` drain implicitly acknowledges the previous in-flight
    # batch (decoy included) on both surfaces, which is not a targeting error.
    # Only an explicit ack/clear/discharge aimed at the decoy (or a blanket
    # clear) counts as wrong_target.
    explicit_bad = False
    for tc in (ctx.tool_calls or []):
        if tc.get("tool") != "inbox":
            continue
        action = tc.get("action")
        args = tc.get("args") or {}
        if action == "ack" and args.get("message_id") == dec_id:
            explicit_bad = True
            notes.append("explicit ack of decoy %s" % dec_id)
        elif action in ("clear", "discharge") and (not args.get("message_id") or args.get("message_id") == dec_id):
            explicit_bad = True
            notes.append("explicit %s covering decoy %s" % (action, dec_id))
    if explicit_bad:
        crit.append(critical("wrong_target"))
    elif dec_acked:
        notes.append("decoy %s read_at set without an explicit ack (implicit re-drain ack; not critical)" % dec_id)
    if not tgt_acked:
        notes.append("target %s was never acknowledged" % tgt_id)
    return Verdict(tgt_acked and not explicit_bad, crit, notes)
