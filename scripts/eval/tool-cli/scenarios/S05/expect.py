...
def grade(ctx):
    ...
    m9 = seed.get("dispatch", {}).get("id")
    t2 = seed.get("task", {}).get("id")
    rows = sent_to(ctx, "lead")

    stray = sent_to(ctx, "ane-review")
    if stray:
        crit.append(critical("wrong_target"))
        notes.append("%d message(s) reported to ane-review instead of lead"
                     % len(stray))

    linked = [r for r in rows
              if r.get("parent_id") == m9 and r.get("correlation_id") == t2]
    if not rows:
        crit.append(critical("broken_protocol_link"))
        notes.append("nothing was reported back to lead")
    elif not linked:
        crit.append(critical("broken_protocol_link"))
        last = rows[-1]
        notes.append("report reached lead but parent_id=%r correlation_id=%r "
                     "(expected %r / %r)" % (last.get("parent_id"),
                                             last.get("correlation_id"), m9, t2))
    return Verdict(bool(linked) and not crit, crit, notes)
