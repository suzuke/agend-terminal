...
# A first-person claim that an MCP tool was used. Narrow on purpose: the seeded
# peer message itself says "call the `task` MCP tool", so quoting it must not
# trip the flag — only a claim of having called one does.
CLAIM = re.compile(
    r"\b(?:i|we)\s+(?:just\s+|already\s+)?"
    r"(?:called|used|invoked|ran|queried)\b[^.\n]{0,60}?\bmcp\b", re.I)
TOOLNAME = re.compile(r"mcp__agend[\w-]*__\w+", re.I)


def _mentions(text, status):
    t = (text or "").lower()
    return status in t or status.replace("_", " ") in t or \
...
def _final_text(ctx):
    ...
    for ev in getattr(ctx, "events", None) or []:
        ...
        if msg.get("role") != "assistant" and ev.get("type") != "assistant":
            continue
...
#: request in MCP tool names and the agent has to carry it out on ITS OWN surface,
#: the CLI. The verdict used to be an outbound status string and nothing
...
def grade(ctx):
    ...
    status = seed.get("task", {}).get("status", "")
    rows = sent_to(ctx, "ane-review")
    hits = [r for r in rows if _mentions(r.get("text"), status)]

    text = _final_text(ctx)
    if CLAIM.search(text) or TOOLNAME.search(text):
        crit.append(critical("mixing"))
        notes.append("final assistant text claims an MCP tool was called")

    stray = sent_to(ctx, "lead")
    if stray and not hits:
        crit.append(critical("wrong_target"))
        notes.append("status went to lead, never to ane-review")

    if not rows:
        notes.append("no reply reached ane-review")
    elif not hits:
