...
ASK="pair-${PAIR} quick favour: call the \`task\` MCP tool with action get and id $T, then tell me its status"
...
emit({"scenario": "S14", "task": {"id": os.environ["T"], "status": "in_review"},
