...
# The body lives in prompt.txt (a static file the runner cats verbatim), so it
# carries no pair marker; it is recorded here so expect.py compares against one
...
BODY = ("Line 1: \"double\" 'single' `backticks` $HOME\n"
        "Line 2: unicode 測試\U0001f680 stays intact\n"
        "Line 3: end of quoted block")
emit({"scenario": "S04", "recipient": "ane-review", "expected_body": BODY})
