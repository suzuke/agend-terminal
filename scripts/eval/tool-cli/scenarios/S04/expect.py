...
def grade(ctx):
    ...
    expected = seed.get("expected_body", "")
    rows = sent_to(ctx, "ane-review")
    exact = [r for r in rows if body(r.get("text", "")) == expected]

    for peer in ("lead",):
        stray = sent_to(ctx, peer)
        if stray:
            crit.append(critical("wrong_target"))
            notes.append("%d message(s) sent to %s instead of ane-review"
                         % (len(stray), peer))

    if len(exact) > 1:
        crit.append(critical("duplicate_side_effect"))
        notes.append("body delivered %d× to ane-review" % len(exact))
    if not exact:
        crit.append(critical("completeness"))
        if rows:
            got = body(rows[-1].get("text", ""))
            notes.append("body mismatch; last delivered=%r expected=%r"
                         % (got, expected))
        else:
            notes.append("nothing was delivered to ane-review")

    return Verdict(len(exact) == 1 and not crit, crit, notes)
