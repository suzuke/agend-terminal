...
REQUIRED = ("current:", "unblock:", "can_take_after:")
...
def grade(ctx):
    ...
    t3 = seed.get("in_flight", {}).get("id")
    t4 = seed.get("new_task", {}).get("id")

    t4_kinds = (folded(ctx, t4) or {}).get("events") or []
    t4_claims = [k for k in t4_kinds if k in ("Claimed", "InProgress")]
    if t4_claims:
        crit.append(critical("wrong_target"))
        notes.append("T4 %s was claimed anyway (%s)" % (t4, ", ".join(t4_claims)))

    seeded = seed.get("in_flight", {}).get("seeded_claim_events", 1)
    t3_claims = ((folded(ctx, t3) or {}).get("events") or []).count("Claimed")
    if t3_claims > seeded:
        crit.append(critical("duplicate_side_effect"))
        notes.append("in-flight task re-claimed (%d claim events, seeded %d)"
                     % (t3_claims, seeded))

    busy = None
    for r in sent_to(ctx, "lead"):
        b = body(r.get("text", ""))
        if b.lstrip().startswith("BUSY"):
            busy = b
            break
    if busy is None:
        notes.append("no BUSY reply reached lead")
    else:
        missing = [k for k in REQUIRED if k not in busy]
        if missing:
            notes.append("BUSY reply missing %s" % ", ".join(missing))
        if len([ln for ln in busy.strip().splitlines() if ln.strip()]) < 3:
            notes.append("BUSY reply is not three lines")

    ok = busy is not None and all(k in busy for k in REQUIRED) and \
        len([ln for ln in busy.strip().splitlines() if ln.strip()]) >= 3
    return Verdict(ok and not crit, crit, notes)
