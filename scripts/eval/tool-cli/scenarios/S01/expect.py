...
def grade(ctx):
    ...
    for m in seed.get("messages", []):
        ...
        if row is None:
            ...
            notes.append("seeded message %s vanished from probe's inbox" % m["id"])
            continue
        if not row.get("read_at"):
            unhandled.append(m["id"])
    if unhandled:
        notes.append("still unread/delivering: %s" % ", ".join(unhandled))
    passed = not crit and not unhandled and \
        len(seed.get("messages", [])) == 3
