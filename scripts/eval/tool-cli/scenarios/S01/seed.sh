...
M1="pair-${PAIR}-msg-1 nightly catalog replay finished clean"
M2="pair-${PAIR}-msg-2 staging cache warmed, no action needed"
M3="pair-${PAIR}-msg-3 release notes draft is in the shared folder"
...
M1="$M1" M2="$M2" M3="$M3" python3 - "$PAIR" <<'PY'
...
msgs = [{"id": mid("probe", os.environ[k]), "text": os.environ[k]}
        for k in ("M1", "M2", "M3")]
emit({"scenario": "S01", "messages": msgs})
