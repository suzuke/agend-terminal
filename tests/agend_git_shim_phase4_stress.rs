...
fn stress_dry_run_only_when_flag_unset() {
    ...
    .ok();
    // Mirror production's `AGEND_WORKTREE_ARCHIVE_FALLBACK` gate.
    ...
    let archive_belt_enabled =
        || -> bool { std::env::var("AGEND_WORKTREE_ARCHIVE_FALLBACK").as_deref() == Ok("1") };
    std::env::remove_var("AGEND_WORKTREE_ARCHIVE_FALLBACK");
    // 100 cutover attempts without the flag → archive belt off, all no-ops.
    for _ in 0..100 {
