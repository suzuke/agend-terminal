...
**Status:** design only, no impl code. Gated behind reviewer4 + dev2 **re-vet**. **REV3** — dev2 side (B4/B5/B6a) CLOSED; reviewer4 §2 re-vet CLOSED B1/B3/B6b and flagged a load-bearing B2 residual (subtractive "explorer-MINUS" keeps `send`, a peer-command amplification vector) → B2 reworked to a positive minimal ALLOW-list + invariants. (REV2 earlier: B4 → per-method default-DENY capability + server-side enqueue target + operator full-cap token; B5 dedup-persist + pre-IPC budget.)
...
1. **dev2 A1 (IPC auth) dominates everything.** The loopback control socket allows every injection-equivalent method (`inject`/`send`/`spawn`/`mcp_tool`/…) by method-shape, not caller-identity, so a spoofed local process drives any bound instance directly — bypassing the sidecar, HMAC, role, and all three pillars. The fix is a **per-method default-DENY capability** model (not connection-auth + a single-method special-case), with the sidecar holding exactly one `enqueue-only` capability and the daemon resolving the target server-side. The operator keeps a boot-minted **full-capability token** so its own CLI/TUI transport is untouched — only the new sidecar surface is narrowed (compat, same pattern as B2). **No inbound adapter ships before this lands.** It is a *pre-existing fleet vulnerability*, fixed first, independently.
...
- **(a) New `RoleKind::Conversational` = explicit positive minimal ALLOW-list (NOT subtractive).** ⟵ REV3. A subtractive "explorer-MINUS" definition **leaks**: the explorer subset (`registry.rs:169-185`) KEEPS `send` and `task`, and **`send` is a peer-command amplification vector** — an unprivileged responder can `send(kind=task, branch, instance=<privileged peer>)`, which routes to `dispatch_auto_bind_lease → bind_full` (`comms.rs:269-279`) and **binds + commands a privileged peer**. Blocker 1 refuses binding the *Conversational itself* but does nothing to stop it *commanding others* → structural unprivilege breaks at "can command the privileged." So define Conversational as a **positive allowlist**: `{reply, inbox, set_waiting_on}` + read-only `binding_state`/`health`, and **explicitly exclude** `send`, `task`, `repo`, `bind_self`, `ci`, `decision`, `config`, and all lifecycle (spawn/delete/restart). (`repo` exclusion also blocks `repo action=checkout bind:true` self-granting a binding.) The exhaustive `match RoleKind` (`registry.rs:203-213`) forces this subset at compile time. NOT reviewer/explorer-based — both retain amplification tools.
- **(a-invariants):** at the RoleKind exhaustive-match seam (`registry.rs:203-213`) assert `send ∉` / `task ∉` / `repo ∉` / `bind_self ∉ Conversational`; plus the Blocker-1 invariant "**no Conversational instance ever has a `binding.json`**."
...
**Touchpoints:** `registry.rs:203-213` (exhaustive-match variant + positive allowlist), `registry.rs:169-185` (explorer subset — the counter-example that KEEPS `send`/`task`, why subtractive fails), `mcp_proxy.rs:280`/`registry.rs:189` (unchanged — compat), inbound router (new target-role gate), `fleet/mod.rs:433` (RoleKind enum).
...
- **(4d) operator transport preserved — narrow ONLY the new surface (compat, same pattern as B2).** The operator's own CLI/TUI drives the control socket via the same direct methods (`spawn`/`delete`/`inject`/`mode`/`shutdown`/`kill`, `api/mod.rs:620-661`); a naïve flip to default-deny would lock the operator out of its own daemon. So: at boot the daemon **mints a full-capability operator token** (0600 in `run_dir`, read by CLI/TUI the same way they read `api.port` today); the per-method capability gate **allows-all for the operator-token holder**. Only the **sidecar** token is narrowed to the single `enqueue-to-responder-inbox` capability. Default-deny applies to *capability resolution*, not to the operator — existing operator transport is untouched; only the new (sidecar) surface is constrained.
**Touchpoints:** `api/mod.rs:620-661` (per-method dispatch — capability check per arm; operator-token = allow-all), `operator_gate.rs:319-326`/`api/mod.rs:204-206` (replace method-shape gate with capability gate), `ipc.rs:47-49` (operator + sidecar token write, 0600, fail-closed), new capability-token + capability-set layer.
...
- **P1a** `RoleKind::Conversational` = positive minimal ALLOW-list `{reply, inbox, set_waiting_on}` + read-only `binding_state`/`health`, excluding `send`/`task`/`repo`/`bind_self`/`ci`/`decision`/`config`/lifecycle; exhaustive-match subset + invariants (Blocker 2a).
