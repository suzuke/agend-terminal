...
- **Discord as a first-class operator channel (#2562, #2586–#2592)** — Discord joins Telegram as a live operator channel. Earlier work shipped REST/gateway-parsing/binding scaffolding but nothing opened a live connection; the gateway WebSocket + `gateway_event_to_channel_event` translation (#2564) is now wired into real daemon/app startup, so a `ChannelConfig::Discord` in `fleet.yaml` constructs a live channel instead of a no-op (#2567), with an inbound dispatcher draining `poll_event()` into the agent inbox (#2587), a write-side per-instance binding (#2588), and a bounded restart supervisor behind a gateway death-status flag (#2590, #2592). Backed by a full inbound/outbound REST + reconnect-backoff test matrix now in the main CI (#2625, #2628, #2629, #2649).
- **Shadow Observer — hook-independent activity inference (#2413, #1523)** — a new out-of-band pipeline infers agent activity without depending on tool-call hooks: an lsof-based API-activity probe (#2426), a local hook-plane reducer with Evidence + quantification (#2433), and generalized `{Hook|Stream}` reducer sources for codex rollout-tail (#2437), opencode SSE (#2440), kiro session-tail (#2447), and an agy lifecycle-hooks plane (#2448). Graduated to default-ON behind a kill-switch (#2449), consumed into the pane badge behind a confidence gate (#2456), and extended to drive operated-state (#2457). The `#1523` turn-completion sentinel shadow measurement now also covers the `claude` backend (#2366, measure-only, default OFF).
- **Multi-channel binding + topic retrofit (#991, #2642)** — a `bind_topic` MCP action retrofits a topic onto a previously-deferred instance (#2598); deployment/team-mode templates propagate `topic_binding` end-to-end (#2600); `list_instances` exposes `topic_binding_mode` (#2606); and `FleetConfig::configured_channels()` gives a canonical multi-channel view (#2643) that Telegram and Discord register through together (#2646).
- **Ephemeral one-shot workers (#1967 Phase 1)** — headless, roster-less PTY workers that spawn, run one turn, and reap: a tracking-store scaffold with day-1 cost guards (#2401), real headless spawn with admission-before-spawn (#2402), a group-kill reap (#2405), and a one-shot driver (inject → idle-debounce turn-end → capture → oracle) shipped for opencode (#2407) then extended to claude (#2408).
- **Per-role MCP capability registry (#2300, #2344, #2367)** — a per-role MCP capability registry (#2344) plus a typed, operator-declared `role_kind` in `fleet.yaml` (seven variants) that drives it (#2367), trimming the tool surface an agent advertises. Read-only roles (reviewer/planner/explorer) now get report/read subsets; an exhaustive match forces a deliberate decision for each new role. Opt-in, default all tools.
- **Three-state inbox delivery (#2345, #2299)** — message delivery moves through `unread → delivering → processed`, with an explicit `inbox ack` and a reclaim-TTL that re-delivers a message whose recipient turn died after the drain (at-least-once) instead of losing it silently.
- **Fugu / Codex model-provider support (#2484–#2492, #2505)** — one-click Fugu quick-spawn (#2487) with model-provider descriptors (#2488) and auto-detection (#2486), Fugu presented as a Codex "Sakana" backend (#2490), a fleet model-tier policy (#2484), and provisioning a real Codex profile instead of an isolated `CODEX_HOME` (#2505).
- **Fleet decision & observation plane (#2524)** — a plan-ack gate requiring pre-work alignment before `in_progress` (#2529), a passive decision badge on the status line + per-pane marker (#2530), decision-board timeout + default handling (#2531), and observation-plane coverage for ephemeral workers (#2532, #2535).
- **Ralph-loop self-continuation + discharge ledger (#2524, #2622)** — a self-continuation loop with a hard `max_iterations` cap (#2543); and a channel-reply / notification discharge ledger (#2541, #2644) wired into the inbox-reclaim and `poll_reminder` chokepoints so real absorption suppresses re-nudges (#2545), fronted by `inbox action=discharge` and a targeted `reply message_id` (#2647, #2648).
- **Off-thread rendering pipeline (Option X — `AGEND_OFFTHREAD_PARSE`, default OFF)** — per-pane parsing can now run off the main thread (#2404), with correctness follow-ups for pane scrolling (#2411), snapshot-based copy selection (#2414), mouse forwarding (#2417), and zoom/resize routing (#2419, #2420).
- **TUI: command palette, image paste, pane dimming** — `Ctrl+B :` command-palette autocomplete for command discovery (#2381) and parameter values (#2387); `Ctrl+B i` pastes a clipboard image — including Finder file-copies via real file-content reads — into agent input (#2435, #2443, #2446); and non-focused panes RGB-blend toward the background by default for easier focus tracking (#2444).
- **Creator-scoped `delete_instance` ACL (#2552)** — an instance's creator can now delete their own spawn (previously orchestrator-only), gated by a `force` + `force_reason` valve when the target has in-flight work.
...
- **Per-tick / render hot-path performance audit (#2348–#2356, #2388–#2400)** — a batch of behaviour-preserving speedups: render redraw-scan throttling and per-pane snapshot-scratch reuse (#2348, #2351, #2354), a PTY-reader pre-hash gate that skips the colour-mask build on unchanged frames (#2349), inbox unread-count via a cheap probe (#2350), `FleetConfig` mtime-cache Arc-sharing (#2356), monitor proc-table reuse (#2355), read-only MCP tools skipping the usage-append / heartbeat RMW (#2388), and task-events compaction hysteresis (#2389, #2400).
- **CI handoff & inbox notification reliability** — reduced CI source-resolution API loopback (#2495), report auto-close routed through the task project board (#2501), a duplicate ci-ready handoff for a terminal PR at the same head suppressed (#2504), a per-workflow notify cursor that silently dropped low-id rerun-to-green transitions fixed (#2520), and pr-state FYI notifications auto-acked on drain (#2508, #2493).
- **Team / project metadata correctness** — `repository_path` forwarded on an already-exists branch instead of dropped (#2525, #2551), an explicit `project_id` override so a fragile `source_repo` slug derivation can't misroute teams (#2509, #2522), and `update-add-member` / `project_id` pinned to survive a daemon restart (#2565).
- **Internal simplification & hygiene** — the `#2050` byte-identical simplification series (dispatch / mcp / overlay / inbox helpers + dead-code removal, #2357–#2365); `AgentState::Thinking` / `ToolUse` merged into a single `Active` (#2461); `quickstart.rs` shrank 2040 → 1100 LOC (#2611); `discord.rs` split into per-concern submodules (#2619); an anti-monolith file-size ratchet + an MCP-docs-vs-registry drift guard (#2483); plus routine dependency bumps including the `quinn-proto` RUSTSEC-2026-0185 security fix (#2416) and `quick-xml` 0.39 → 0.41 dropping two RUSTSEC ignores (#2651).

### Fixed

- **Canonical-worktree deletion incident — root cause + detection (#2668, #2669)** — the 2026-07-06 canonical-repo deletion traced to `repo release` running `git worktree remove --force` (which git refuses for a main tree) and then falling through to an unconditional `remove_dir_all`. `validate_release_path` now refuses any primary / main working tree using git itself as the source of truth (not filesystem heuristics), so bare repos and `--separate-git-dir` mains can't slip through (#2668); and a canonical heartbeat + `binding_state` liveness check makes any future canonical loss loud instead of silent for 40 minutes (#2669).
- **Four data-loss defenses (#2672, #2673, #2677, #2679)** — `bind_self` re-provisioning no longer orphans an existing `origin/<branch>` (#2673); both manual worktree-release paths snapshot dirty WIP into a recovery ref instead of discarding it unconditionally (#2672); the `agend-git` shim requires `--force-with-lease` for feature-branch force-pushes, closing a footgun where any agent could silently overwrite another agent's origin branch (#2677); and a closing task settles its `dispatch_tracking` rows (not just its `dispatch_idle` sidecars), so `sweep_stuck` stops nagging about already-closed work (#2679).
- **Security & reliability audit sweep — 17 root-cause fixes (#2510)** — an SSRF / token-exfiltration gate restricting `ci-watch` forge tokens to trusted hosts (AUDIT2-001), per-caller ACL on destructive tools (AUDIT2-002/003), a cron DST fall-back re-fire storm stopped (AUDIT2-010), atomic + locked runtime-config writes (AUDIT2-012), and panic-isolated crash-event dispatch (AUDIT2-007), plus assorted TUI / tasks / skills / durability fixes.
- **Render-loop freeze series** — OWNED restore spawns deferred to a render-first background pool on restart (#2343); redraw capped to 30 fps with coalesced wakeups (#2346); a lock-free published `AgentState` mirror ending `core.lock` contention freezes (#2380); `drain_output` capped per frame to stop PTY-backlog draw freezes (#2385); and restart floods absorbed into a bounded boot / loading phase (#2396).
- **Rate-limit / server-overload recovery, continued** — the self-clear latch resets on a fresh `ApiError` hook (#2415, continuing #2318 / #2319); a stale `ServerRateLimit` badge yields to fresh post-SRL hook activity (#2470, #2471); and a spike quantified that agents essentially never self-clear a rate-limit block (2746 / 2746 shadow entries showed `self_cleared=false`), so the dead `rate_limit_self_cleared` signal and the ineffective self-clear instruction were removed — real recovery now rides the hook-authoritative path (#2674, #2675).
- **Worktree / binding / dispatch race integrity** — a typed `LeaseError` replaces a lease that could silently return `Ok` with a missing binding (#2464); a safe same-agent rebind repair replaces a destructive fallback (#2496, #2523); stale dispatch-checkout bindings now fail closed (#2500); push guards resolve the real default branch instead of hardcoding `origin/main` (#2662); `HourlyGc` and worktree-registry sweeps moved off the daemon's main tick loop (#2614, #2616); and three worktree-reclaim false-positive classes were closed (#2657).
- **Dirty canonical main working-tree guardrail (#2512)** — a stray untracked file could land in the canonical main working tree past every existing guard (the agend-git shim only sees git commands; drift checks only look at HEAD state; `.gitignore` matched only the exact filename); a new L1 + L2 detector (`apply_to_canonical` → `Option<CanonicalDirtyReport>`) closes that exact gap.
- **Health / respawn hardening (#2480, #2538)** — a process-liveness gate is now enforced on decay transitions and `respawn_ok`, crash-respawn failures escalate instead of silently stalling, and backend-exit foreground-identity detection fires a `backend_exited` notify + Unhealthy transition (#2546).
- **PTY write-actor race & leak hardening** — an fd-reuse race fixed with per-writer thread isolation (#2620, #2630), an idle busy-poll replaced with park-on-empty-queue (#2656), and fd-leak regression guards + a ConPTY sideload audit added (#2613).
- **Inbox delivery reliability (#2299)** — poll-reminders re-arm when reverting stale `delivering` rows (#2362); `mark_ci_watch_superseded` matches on `correlation_id` equality instead of a text substring (#2370); poll-reminder counts only real obligations, not already-drained reports (#2412); and `DELIVERING` settles on session-reset via an `ack_inbox` send (#2425).
- **dispatch_idle zombie / quota-wedge messages (#2676, #2678)** — reclaiming a rate-limited agent now also clears its `dispatch_idle` sidecar, so a released reviewer / query dispatch stops firing stale watchdog alerts (#2676); and the quota-wedge escalation is durably one-shot instead of self-clearing on a snapshot flicker that had caused duplicate fires minutes apart (#2678).
- **Bug-audit hardening (#2368, #2372)** — RAII connection-slot guards so a panicking `handle_session` can't leak the counter to its 32-slot cap and lock up the control plane (#2368), and `validate_args` rejects a present-but-JSON-null required field as missing rather than silently forwarding an empty string (#2372).
- **Auth & authorization (#2369, #2378)** — Telegram's allowlist check now runs before fleet-status injection and `加 task:` board writes (#2369), and reviewed authz content is no longer misflagged as an `AuthError` (#2378).
- **Telegram robustness** — quote-reply correspondence is now precise via a persistent sent-message ledger instead of best-effort matching (#2570), TUI pane create / kill hooks fire again after a post-#945 channel-ref regression (#2591), and a topic-reuse identity-confusion hole is closed (#2593).
- **send validation false-reject (#2681)** — the reviewer SHA-staleness gate scanned only `summary` for the PR URL while the sibling evidence gate scanned `summary + artifacts`, so a verdict carrying the URL in `artifacts` was false-rejected into the fallback-inject path; both gates now share one scan surface, and the no-URL reject message is one-shot-actionable.
- **e2e local-precondition friction (#2680)** — two real-daemon e2e tests now skip, rather than fail, when local auto-bind isn't available, so a missing local precondition no longer reads as a false CI failure.
- **GR1 binding-hijack observability (#2158, #2341, #2361, #2373)** — a self-claim bind (no task dispatch) now surfaces to the operator and no longer silently auto-arms a ci-watch (#2341); the notice routes to the bound agent's team orchestrator instead of the global operator inbox (#2361); and the per-branch fire-once latch clears on release so a later real re-claim of the same branch re-surfaces (#2373). Detection-only — push authority stays HMAC- + guard-gated.
- **Split-pane & local-shell restore (#2359, #2360)** — operator-opened local / scratch shells (`Ctrl+B c`) are exempt from the #1441 unmanaged-spawn gate (#2359), and a restored split pane resizes to its content rect after the deferred attach so it no longer stays at the wrong size (#2360).
...
- **Protocol document rename (historical path `docs/FLEET-DEV-PROTOCOL-v1.md` → live `docs/FLEET-DEV-PROTOCOL.md`)** — drop the `-v1` suffix. The document is internally versioned in its header (`v1.2`) and no v2 is in flight; the path-level `-v1` was a 2025-era misnomer suggesting a parallel v2. Compile-time `include_str!` in `src/protocol.rs`, the `Cargo.toml` `[package].include` whitelist, the `tests/cargo_include_invariant.rs` mock-pattern filter, the README link, and the now-retired architecture quick-start and lint-discipline records were updated; retired documentation is covered by [Historical records](docs/README.md). Operators with hand-edited overrides at the old path must rename manually (no auto-migration; mechanism rarely used).
...
- **`cargo install agend-terminal` build failure on 0.4.0** — `src/protocol.rs` used `include_str!("../docs/FLEET-DEV-PROTOCOL-v1.md")` against that release's historical protocol path, but the file wasn't in the `Cargo.toml` `include` whitelist. The packaged tarball that `cargo publish` ships to crates.io was therefore missing the bundled protocol doc, and verification compile failed with "No such file or directory". GitHub Release binaries (built from the source tree, not the packaged tarball) were unaffected, so v0.4.0's binary downloads still work — but there is no v0.4.0 on crates.io. v0.4.1 is identical to v0.4.0 in source apart from this single packaging fix.
...
- **Windows-support Phase A** — `nix` dependency removed; file locks via `fs2`, PID helpers via `src/process.rs` with platform-conditional `libc` / `windows-sys` impls; `/tmp` hardcoding replaced with `dirs::home_dir()` / `std::env::temp_dir()`. UDS remained the last Windows blocker; the retired Windows plan is covered by [Historical records](docs/README.md).
