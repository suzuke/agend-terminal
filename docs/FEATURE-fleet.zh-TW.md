...
**1. 編輯 fleet.yaml**——在 `instances` 區塊下新增一筆，再重啟 daemon：
...
**2. TUI app 模式**——在 `agend-terminal app` 中使用內建 UI，以互動方式建立新的 instance，不必編輯 YAML。

**3. MCP 工具**——任何執行中的 agent 都能透過 `create_instance` MCP 工具以程式方式建立新 instance，適合自動化協調與 deployment template。
