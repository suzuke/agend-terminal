...
**可控的清理。** 任務完成且 PR 合併後，daemon 對 worktree 執行 soft release（標記 `released_at`）。Worktree 會在磁碟上保留 24 小時寬限期。如果沒有人重新認領，GC 會自動移除（其封存到 `.trash` 的 fallthrough 補救帶由 `AGEND_WORKTREE_ARCHIVE_FALLBACK=1` 控管，此名由已棄用的 `AGEND_WORKTREE_GC` 更名而來）。對於卡住或孤立的 worktree，operator 可以使用 `release_worktree(force:true)` 作為緊急手段。
