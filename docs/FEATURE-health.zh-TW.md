...
**Hang 偵測與恢復。** Agent 收到任務但在 `Thinking` 狀態下超過 600 秒沒有 PTY 輸出。若 hang auto-recovery 已啟用，daemon 會送出一次受保護的 ESC interrupt 並觀察結果。原本自動 Stage 2 restart 與 Stage 3 pause/escalation 階梯已不再屬於 Hung recovery；Stage 1 未成功時會留下 log，交由 operator 處理。
...
| `Paused` | 另一個 lifecycle handler 已達重試上限，等待 operator 手動介入 | 否（終態） |
...
```json
{
  ...
  "reason": "rate_limit",
  "retry_after_secs": 60,
  "note": "預期 provider 很快 reset"
}
```

`report` 會套用到已驗證的 caller instance。支援的 reason 為 `rate_limit`、`quota_exceeded`、`awaiting_operator`；`retry_after_secs` 與 `note` 為可選。Agent 可以主動回報問題，讓 watchdog 知道不需要觸發 hang 偵測。

### health clear

清除明確 instance 先前設定的 blocked reason，恢復正常 hang 偵測：
...
`instance` 必填。`reason` 是可選 match filter；若與目前 blocked reason 不相符，操作會失敗，不會清除無關狀態。
...
轉為 Healthy。但在 Failed 期間程序已停止，operator 可以明確重啟；
crash count 自然衰減之後也可能允許正常 lifecycle recovery，但沒有 Hung Stage 2 auto-restart 可以等待。
