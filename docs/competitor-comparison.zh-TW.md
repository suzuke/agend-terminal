...
版圖已遠遠超出原文件的兩個對象。市場現在分成九大類（§3）。誠實的張力：AgEnD 自承的三個最大缺口
...
`render/`、`layout/`、`keybinds.rs`）與一條協調脊柱（32 個 MCP 工具、event-sourced
...
| **多 runtime 助理平台** | **DuDuClaw** (~22★, Rust + Python + React, Open-Core) | worktree + opt-in 容器 | 以基本元件論是最接近的 Rust-daemon 同類（PTY pool + worktree + MCP board），但是 chatbot/助理平台——coding 編排預設關閉；出貨一個值得照搬的 in-band sentinel-framing PTY turn 偵測器 |
...
- **模型：** 找到的架構*最接近*同類。一個 Rust daemon（21-crate workspace）透過 **PTY pool**
  （`crates/duduclaw-cli-runtime`）驅動 Claude/Codex/Gemini CLI，用 **per-(agent,task) git
  worktree**（「L0」，`crates/duduclaw-gateway/src/worktree.rs`）+ opt-in 容器層（「L1」）隔離，
  並透過 **SQLite task board** + **MCP server（80+ 工具）** 協調。但它的重心是**多通道 AI
  助理/chatbot 平台**（7 通道、持久記憶、人格、語音、本地推論、self-evolution、web dashboard）
  ——coding-agent 編排只是旁支、**預設關閉**（`agent.toml [runtime] pty_pool_enabled`）。無 TUI；
  跨 agent IPC 是檔案佇列（`bus_queue.jsonl`），非結構化 review chain。年輕（2026-03 建立）、單一
  作者、轉 Open-Core。
- **可借鑒（亮點）：PTY turn 偵測的 in-band sentinel framing**
  （`crates/duduclaw-cli-runtime/src/envelope.rs`）。不刮 scrollback 去猜 CLI 一回合何時結束，而是
  （透過 system-prompt）注入指令要 CLI 把最終答案包進唯一 magic sentinel
  （`=====DUDUCLAW.RSP.<uuid>.MARK=====`），再 `strip_ansi` + buffer 累積 + 配對，抽出精確 payload，
  並用 chrome filter 丟掉 spinner/狀態行。這是修 AgEnD #1 弱點（脆弱的「眼睛」）的**第三條路**，與
  ACP（§5.6）、stream-json（§4.3）並列——而且是唯一**不需 backend 協定支援**的，只要一個
  system-prompt 約定。見 §9 Tier 1.1。值得照搬的踩雷細節（2026-05 spike）：互動模式拿掉 UUID
  （Claude TUI 會吃掉開頭 sentinel 的一個字元），改成每回合清空 buffer 後用位置配對；別用 `<…>`
  分隔符（TUI markdown renderer 當成 HTML 吃掉）；strip 掉 TUI 在每個字元間插入的 `ESC[1C`
  cursor-forward，sentinel 位元組才連續。其他借鑒：PTY pool 快取鍵含 `model` + `account_id`
  （他們踩過 per-agent model 被靜默丟棄的 bug——對應 AgEnD #2038）；worktree 上限（5/agent、20
  total）+ 形容詞-名詞分支名（`wt/{agent}/{adj}-{noun}`）；獨立程序的 PTY worker（`/healthz` +
  loopback Bearer）隔離 PTY crash；以及明確的 L0-worktree → L1-container 分層隔離（對應 §9 Tier 1.2）。
...
| 缺口 | 為何刺痛 | 誰做得更好 |
|---|---|---|
| **脆弱的 PTY 螢幕刮取** 做 state detection（「眼睛」） | regex 跨 backend 版本漂移 → 誤判 hang/idle、誤觸 recovery | Crystal (stream-json)、OpenHands (event stream)、container-use (structural)、ACP、tmuxai (OSC-133)、DuDuClaw (sentinel framing) |
...
1. **結構化 / event-based state，PTY 刮取降為 fallback。** 加一個 ACP backend 模式，*並*先把 ACP 的
   ...
   (Crystal) / 一個 ACP harness (Vibe Kanban `acp/harness.rs`) 餵 high-confidence 訊號。對**既不說
   ACP 也不說 stream-json** 的 backend，**免協定變體**是 DuDuClaw 的 *in-band sentinel framing*
   （§4.6）：注入 system-prompt 約定把最終答案包進唯一標記，再 strip-ANSI + 配對取代刮取。*來源：*
   ACP、Crystal、OpenHands、container-use、tmuxai、VK、DuDuClaw。**工時高 · 契合 yes。** *攻擊缺口 #1——單一
   最高槓桿的改動。*
