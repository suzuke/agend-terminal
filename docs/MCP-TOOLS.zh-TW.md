...
- 不帶參數會 drain unread 訊息並標為 `delivering`；此時尚未標為 processed。回傳 row 包含 durable `delivery_count` 與 `first_delivered_at`，redelivery 的 structured header 也會顯示次數，但不改變 canonical identity/text。
...
- 再次 drain 會隱式 ack 前一批 delivery；未確認的 batch 約十分鐘後可能被 reclaim 並重新投遞。Fresh session reset 也會重新排入未確認 row，交由 successor recovery。
