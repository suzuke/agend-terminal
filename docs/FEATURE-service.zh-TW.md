...
Systemd user manager 通常會在使用者最後一個 login session 結束後停止。Headless
或 always-on Linux host 在 install 後應執行一次：
...
這會在 boot 時啟動 user manager，並讓 service 跨 logout 持續運行。對目前使用者
不需要 root，而且具 idempotency。若要退出此行為，執行
`loginctl disable-linger`；若要移除 service 本身，使用
`agend-terminal service uninstall`。macOS launchd 與 Windows at-logon task
沒有等價的額外步驟。
...
| Command | 沒有 daemon 運行 | 已有 daemon 運行 | Shell 行為 |
|---|---|---|---|
| `agend-terminal start` | spawn detached daemon；parent 結束 | 拒絕第二個 daemon | non-blocking |
| `agend-terminal start --foreground` | foreground 運行 daemon | 拒絕第二個 daemon | blocks；Ctrl+C 停止 |
| `agend-terminal start --agents NAME:CMD ...` | 隱含 foreground mode、略過 `fleet.yaml`，只啟動列出的 agent | 拒絕第二個 daemon | blocks |
| `agend-terminal app` | **Owned**：TUI 擁有 in-process daemon | **Attached**：TUI 是既有 daemon 的 client | blocks |
| `agend-terminal tray` | resident UI；Start action shell out 到 `start --foreground` | 只反映 daemon state，不再 spawn | resident |

`bootstrap::prepare()` 先 probe run directory、取得 exclusive daemon lock，再做一次
probe 作為 TOCTOU guard，最後回傳 `Attached` 或 `Owned`。這帶來一個重要的
operator asymmetry：離開 cold-start 的 Owned `app` 會一併結束 daemon 與 PTY；
離開 Attached `app` 只會關閉 TUI。Issue #879 已關閉 always-Attached migration
嘗試；上述 asymmetry 是目前行為。

Tray 必須保持在 daemon internals 之外。它的 Start action 執行
`agend-terminal start --foreground`，不會取得 daemon lock，也不直接呼叫 API。
Detached spawn 同樣以 `--foreground` re-exec，避免 child 再進入 detach branch
造成遞迴。

其他 lifecycle-adjacent commands 是 `attach <name>`（連到既有 PTY）、
`connect <name> --backend X`（暫時 external backend）、`stop`、`kill <name>`，
以及 `list`（`status` / `ls`）。
