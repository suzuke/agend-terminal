...
- **Deny matrix on agent git ops.** The shim refuses certain commands from unbound or cross-branch contexts: `git worktree add/remove/move`, `git checkout` of a different branch, etc. The daemon owns the worktree pool. Current behavior is defined by the live guard and [`FLEET-DEV-PROTOCOL.md`](FLEET-DEV-PROTOCOL.md) §10, §12.4, and §13; the original proposal remains available through the [historical-record retrieval rule](README.md#historical-records).
...
- [Historical-record retrieval](README.md#historical-records) — recover the original shim design when investigating Phases 1–5
