...
本文件分門別類列出常用且受支援的 operator-facing `AGEND_*` 環境變數，外加會被尊重的
外部／標準變數，以及僅供測試使用的 fixture。程式碼仍是權威來源；新加入的內部 seam
可能會先出現在原始碼，之後才補進本指南。
...
這些變數由 vendored `agentic-git` shim 讀取。`AGENTIC_GIT_*` 是正式名稱；對應的
`AGEND_*` 名稱仍作為 legacy fallback 受理。三個 `AGENTIC_GIT_BYPASS*` 控制項是
層層疊加的緊急覆寫——全部都是 ⚠️ security-sensitive。

| Name | Purpose | Default (unset) | Valid values / format | Source | Notes |
|------|---------|-----------------|-----------------------|--------|-------|
| `AGENTIC_GIT_BYPASS`<br>`AGEND_GIT_BYPASS`（legacy） | Layer-1 一次性覆寫：若有設定，`should_bypass()` 回傳 true 並 exec 真實 git，跳過所有強制執行。 | 不 bypass。 | **以存在判定**（`is_ok()`）；任何值，甚至空字串。 | `vendor/agentic-git/crates/agentic-git/src/lib.rs:541` | ⚠️ 緊急覆寫。Daemon 內部的呼叫者會設 `=1` 來跳過 shim。`AGEND_*` 拼法作為 legacy fallback 受理。 |
| `AGENTIC_GIT_BYPASS_AGENT`<br>`AGEND_GIT_BYPASS_AGENT`（legacy） | Layer-2 特定 agent 的豁免：當值等於目前 agent 身分時 bypass。 | 無 agent 豁免。 | Agent/instance 名稱字串。 | `vendor/agentic-git/crates/agentic-git/src/lib.rs:544` | ⚠️ 會與目前 agent 身分做值比對。`AGEND_*` 拼法作為 legacy fallback 受理。 |
| `AGENTIC_GIT_BYPASS_UNTIL`<br>`AGEND_GIT_BYPASS_UNTIL`（legacy） | Layer-3 有時限的豁免：在 now < 指定 epoch 之前 bypass。 | 無時間豁免。 | **Unix epoch 秒數**（`u64`，非 ISO）。 | `vendor/agentic-git/crates/agentic-git/src/lib.rs:551` | ⚠️ 過期／無法解析 → 不 bypass。`AGEND_*` 拼法作為 legacy fallback 受理。 |
| `AGENTIC_GIT_SHIM_DEPTH`<br>`AGEND_GIT_SHIM_DEPTH`（legacy） | 遞迴防護，會傳遞到 spawn 出來的 git；在 `MAX_SHIM_DEPTH = 3` 時 hard-fail。 | `0`。 | 非負 `u32`；無法解析 → 0。 | `vendor/agentic-git/crates/agentic-git/src/lib.rs:67`（讀取）；於 `vendor/agentic-git/crates/agentic-git/src/exec.rs:22,53` 設定 | 內部 sentinel；正常情況下不由使用者設定。`>= 3` 時 exit 70（#1504）。`AGEND_*` 拼法作為 legacy fallback 受理。 |
| `AGENTIC_GIT_REAL_GIT`<br>`AGEND_REAL_GIT`（legacy） | escape hatch，存放真實 git 二進位檔的路徑，讓 shim 能 exec git 而不遞迴。Daemon 在 agent spawn 時注入。 | Shim：未設定 → 退回字面 `"git"`，再走 PATH-exclude 解析。Daemon：只在尚未設定時注入。 | 指向 git 可執行檔的絕對路徑；只有檔案存在時才接受。 | `vendor/agentic-git/crates/agentic-git/src/exec.rs:87`（讀取）；daemon 於 `src/agent/mod.rs:1160-1174` 注入 legacy `AGEND_REAL_GIT` | ⚠️ Correctness-sensitive：錯誤／缺漏的值有遞迴 spawn 風暴的風險（#1504）。`AGEND_*` 拼法作為 legacy fallback 受理。 |
| `AGENTIC_GIT_HOME`<br>`AGEND_HOME`（legacy） | agentic-git shim 自己的 home：shim 會把這個目錄的 `bin/` 從交給真實 git 的 PATH 中排除，並把稽核紀錄寫在這裡。Daemon 在 agent spawn 時把它釘在 daemon home（#1504 家族）。 | Shim：未設定 → 退回 `AGEND_HOME`。Daemon：只要知道 home 就會注入。 | 絕對目錄路徑，等於注入的 `AGEND_HOME`。 | `vendor/agentic-git/crates/agentic-git/src/exec.rs:110`（讀取）；daemon 於 `src/agent/mod.rs:941` 注入 primary `AGENTIC_GIT_HOME` | ⚠️ 列於 `SENSITIVE_ENV_KEYS`：fleet.yaml／template env 無法改寫它。沒有這個釘住值時，把 `AGEND_HOME` 限定到暫存目錄（`ScopedAgendHome`）再 spawn `git` 的測試會讓 shim 解析到自己並觸發遞迴防護（agent shell 內約 40 個必定失敗；CI 的 PATH 上沒有 shim）。 |
...
| Name | Purpose | Default (unset) | Valid values / format | Source | Notes |
|------|---------|-----------------|-----------------------|--------|-------|
| `AGEND_HOOK_STATE_POC` | 啟用舊版 per-workspace Claude hook-event 回報器。其 store 仍供 inject-delivery watchdog、ServerRateLimit hook override 與 hook-event 診斷使用；原本的 `authoritative_state` snapshot 推升已在 #2413 移除。 | 關閉（不注入舊版回報器）。 | 以值判定：剛好 `"1"` 啟用；否則關閉。 | `src/mcp_config.rs`（`hook_settings_json`／`hook_settings_with_base`）；`src/daemon/hook_shadow.rs` | 舊版、預設關閉的資料層；不要和下方預設開啟的多 backend Shadow Observer 混淆。 |
| `AGEND_SHADOW_OBSERVER` | 多 backend Shadow Observer 的總 kill-switch。啟用時，Hook／Stream evidence 會被 reduce 成 `observed_status`，高信心修正可供下方 operated-state consumers 使用。 | **開啟**。 | 只有字面值 `"0"` 會停用；未設定或其他值都啟用。 | `src/daemon/shadow/mod.rs`（`enabled`） | `=0` 會停用 observer tick、thread、spawn-env injection 與 operated correction。即使啟用，health／hang／recovery decider 仍刻意使用 raw state。 |
| `AGEND_OBSERVED_DISPATCH` | 控制高信心 Shadow Observer 修正是否可透過 `operated_state` 驅動 snapshot／dispatch、poll-reminder、reclaim 與 API injection consumers。 | Shadow Observer 啟用時**預設開啟**。 | 只有字面值 `"0"` 會停用；未設定或其他值都啟用。 | `src/daemon/shadow/mod.rs`（`operated_dispatch_enabled`、`operated_state`） | 行為修正的獨立 rollback；`AGEND_SHADOW_OBSERVER=0` 也會一併停用。它永不改寫 raw screen-derived state。 |
...
| Name | Purpose | Default (unset) | Valid values / format | Source | Notes |
|------|---------|-----------------|-----------------------|--------|-------|
| `AGEND_ENV_ISOLATION` | agent-backend env 隔離的閘門（#1440 分階段推廣）。 | 停用。 | `"1"` 啟用；否則關閉。 | `src/agent/mod.rs:179` | 預設關閉的功能旗標。開啟時，只有 allowlist 中的 env 會被轉發給 backend（見 [external env](#15-honored-external-env)）。 |
...
| `AGEND_CI_TRUSTED_HOSTS` | 以逗號分隔、可接收 CI／forge token 的額外 hostname（供 self-hosted GHE／self-managed GitLab 使用）。安全閘門：agent 可透過 `ci watch` MCP 工具設定 `ci_provider_url`，因此 daemon 只會把 token 附加到內建 SaaS host（`api.github.com`、`gitlab.com`、`api.bitbucket.org`）、loopback，以及此處列出的 host——絕不傳給任意由 agent 提供的 host。 | 只信任內建 SaaS host + loopback；其他 host 一律以**未認證**方式輪詢。 | `src/daemon/ci_watch/provider.rs`（`host_receives_credentials`） | 監看 self-hosted forge 時請設定，例如 `AGEND_CI_TRUSTED_HOSTS=ghe.corp.example,gitlab.internal`。可防止 CI token 外洩（原始 SSRF 會讓 token 隨請求送往任意 URL）。 |
...
`grep -rhoE 'AGEND_[A-Z0-9_]+' src/` 也會撈出一些**並非** runtime 環境變數的識別字。
以下例子說明常見的 false positive；它們不是上方 operator-facing 表格的完整性證明。
