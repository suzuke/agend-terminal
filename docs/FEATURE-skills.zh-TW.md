...
**Operator 安裝 code review 技能。** 你在 GitHub 上找到一個社群維護的 code review 技能。執行 `agend-terminal skills add https://github.com/user/code-review-expert.git` 一次，Claude、Codex、OpenCode、Kiro、Antigravity 與 Grok agent 都能使用。不需要逐一設定 backend。
...
不同的 AI backend（Claude、Codex、OpenCode、Kiro、Antigravity 與 Grok）各有自己的技能目錄慣例（`.claude/skills/`、`.codex/skills/` 等）。手動在每個目錄下維護相同的技能檔案既繁瑣又容易不同步。

Skills 系統提供一個統一的來源目錄 `$AGEND_HOME/skills/`，透過 symlink 自動映射到每個 backend 的慣例路徑。安裝一次，六個 backend 同時生效。
...
2. 在 `$AGEND_HOME/.skills-stage/<digest>/` 建立篩選後的副本
