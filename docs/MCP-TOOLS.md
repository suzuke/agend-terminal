...
- Required: `message`. Plain text, truncated at 1000 characters, always prefixed with the calling instance's name. Before the cap and the prefix the body is flattened to ONE line — every control character (LF, CR, TAB, VT, FF, NEL, the rest of C0/C1) plus `U+2028` LINE SEPARATOR and `U+2029` PARAGRAPH SEPARATOR becomes a space, and runs of spaces collapse — and any literal `[operator-page from ` inside the body is rewritten to `[quoted: operator-page from `. So exactly one `[operator-page from …]` marker reaches the operator and it is the daemon's own. The honest limit: a client that soft-wraps a long page can still start a visual row mid-body, and a body is still free to contain other sender-looking prose. This stops the marker being reproduced and being put on a line of its own; it does not make a page unimpersonatable.
