...
> 本文件為非規範性繁體中文翻譯；若本翻譯與英文版有任何不一致，應以英文版為準。
...
- 一般 send 回應使用誠實的 admission 標籤：`pty` 僅保留給明確的 `LegacyPty` 相容路徑；`transport_queued_unverified` 表示 durable inbox row 已記錄，但 worker 的 adapter route 與 backend attempt 仍未驗證。durable transport receipt 會保留實際路由、attempt 編號與結果（`queued`、`accepted`、`observed_in_session`、`turn_started`、`completed`、`failed`、`ambiguous` 或 `ack_overdue`）；這些標籤都不代表接收者已閱讀、理解或確認訊息。
