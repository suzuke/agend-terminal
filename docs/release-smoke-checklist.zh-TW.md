...
- [ ] 沒有上一個 session 殘留的 daemon 在跑：`agend-terminal stop`（或確認 `agend-terminal list` 沒有任何輸出）
- [ ] 工作目錄是 repo 根目錄
- [ ] build 是最新的：`cargo build --release`（或 CI artifact 對應到目標 commit）
- [ ] `AGEND_HOME` 解析正確：`agend-terminal doctor` 以 0 結束
- [ ] 若要測試 Telegram channel：`AGEND_TELEGRAM_BOT_TOKEN` 已設定且 bot 在線上
...
- [ ] **Spawn** — `agend-terminal start --agents claude:claude` — 就緒提示字元（`❯` 或 `bypass permissions`）在 **30 秒** 內出現
...
- [ ] **Branch 預覽** — `agend-terminal admin cleanup-branches` 以 0 結束（預設只預覽，不刪 branch）
...
### 1e. Antigravity CLI（`agy`，註冊為 backend `antigravity-cli`）— #987

Gemini CLI 的官方接班人（Gemini CLI 將於 2026-06-18 對 free／Pro／Ultra 方案停止服務）。binary 命令是 `agy`；fleet 中的顯示名稱是 `antigravity-cli`（#995）。

- [ ] **Spawn** — `agend-terminal start --agents agy-smoke:agy` — 就緒提示字元（`Antigravity CLI` banner 或 `Type your message`）在 **20 秒** 內出現；workspace-trust 對話框被自動關閉（#995 dismiss_pattern）
- [ ] **Echo** — 注入 `echo hello` + Enter；看得到回應
- [ ] **Tool use** — 注入 `list files in /tmp`；觸發 tool 行為
...
- [ ] **Spawn** — `agend-terminal start --agents grok-smoke:grok` — `Grok Build`／`❯` 在 **30 秒** 內出現；project-trust 對話框被自動關閉
- [ ] **Echo** — 注入 `echo hello` + Enter；看得到回應
- [ ] **Tool use** — 注入 `list files in /tmp`；觸發 tool 行為
- [ ] **Quit** — 注入 `/exit` + Enter；pane 關閉；沒有殘留的孤兒 `grok` process

---

## 2. 跨功能測試（≤ 5 分鐘）

- [ ] **鍵盤導覽** — `Ctrl+B n`／`Ctrl+B p` 切換 tab；`Ctrl+B o` 切換 pane；`Ctrl+B d` 乾淨地 detach
- [ ] **滑鼠滾輪捲動** — 在一般（非 alt-screen）pane 中，滑鼠滾輪能捲動 vterm 歷史
- [ ] **Telegram channel 綁定** — `agend-terminal start`；透過 Telegram 送一則訊息；daemon 把它路由到正確的 agent pane（需要 `AGEND_TELEGRAM_BOT_TOKEN`）
- [ ] **Worktree lease／release** — 從 agent 對 disposable branch 呼叫 MCP `repo(action=checkout, bind=true)`，再呼叫 `release_worktree`；`binding_state` 回傳 `bound:false` 且沒有 dangling worktree
- [ ] **被動擷取 opt-in** — 設定 `AGEND_CAPTURE_FIXTURES=1`，跑一個 backend 冒煙測試區塊，確認 `$AGEND_HOME/captures/<agent>/` 內含一對 `.cap` 與 `.cap.meta.json`，然後 `unset AGEND_CAPTURE_FIXTURES`
