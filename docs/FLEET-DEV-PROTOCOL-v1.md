...
#### 3.5.10 External-fixture validation for wire-protocol PRs

Sprint 25 P0 amendment. Any PR touching cross-process IPC, stdio framing, or network protocol code (`src/bin/`, `src/api/`, `src/channel/`, `src/mcp/`, `src/daemon/lifecycle.rs`, `src/daemon/task_sweep.rs`, etc.) **must** include at least one test exercising a payload captured from — or bit-for-bit replicating — a real-world client/server interaction.
...
When reviewing a PR with scope-listed file changes touching the protocol layer, reviewer **must** verify presence of an external-fixture test. Absent → flag as `EXTERNAL-FIXTURE-ABSENT — UNVERIFIED unless explicitly waived with rationale`. Operator self-merge requires explicit fixture justification in the commit message.

##### Exemption

Refactor PRs that demonstrably preserve byte-output (e.g. moving framing code between modules without changing wire format) may waive external-fixture if they include a regression test asserting byte-equivalence against the pre-refactor output.
