...
Channels 系統讓操作者直接在 Telegram 或 feature-gated Discord adapter 中與 agent 互動，不需要開啟終端。Telegram 為每個 agent 對應 forum topic；Discord 則透過設定的 channel mapping 路由 gateway 訊息。
...
- `bot_token_env`：存放 @BotFather token 的環境變數名稱
...
核心程式碼只透過 trait 操作，不直接呼叫 Telegram API。Telegram 與 feature-gated Discord adapter 都實作此介面。
...
`fleet.yaml` 目前選擇一個 tagged `channel:` 設定（`type: telegram` 或 `type: discord`）。runtime registry 不綁平台，但 schema 尚不能同時設定多個 adapter。Discord 需要 `discord` Cargo feature，以及 `AGEND_DISCORD_BOT_TOKEN`（或自訂的 `bot_token_env`）。
...
Discord 已在 `discord` feature gate 後實作。Bootstrap 會啟動 live gateway，把通過 allowlist 的 `MESSAGE_CREATE` 事件映射到 inbox，並註冊 adapter 供 outbound 操作使用。它尚未實作 Telegram 的 `fleet_binding`／`UxEventSink` 路徑，因此 fleet-wide UX event mirror 仍只支援 Telegram。
...
- `src/fleet/mod.rs`：fleet 與 instance metadata，含 topic 欄位
