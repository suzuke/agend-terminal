...
- **worktree 生命週期由 daemon 管理。** 清理是透過 `release_worktree` MCP 工具進行，而不是直接 `git worktree remove`。daemon 端每小時的 GC 掃描（`gc_tick`）接著會**自動移除**那些已 release、超過 grace period、且未被 pin 或 bind 的受管理 worktree——force-reclaim 的候選會被歸檔到可復原的 `.trash`、而非硬刪，過期的 ci-watch lock 也會一併清掉。（被移除或歸檔的 worktree 會連同它的 `target/` 一起處理;但此掃描不會**獨立**掃描或清除 build artifact 目錄——那是另一個 disk-GC 的範疇。）若只想預覽會被移除什麼而不實際刪除,可使用 `gc_dry_run` MCP 工具。
