...
| 10 | Transactional worktree lifecycle | in progress | Managed release 與 branch retirement 現已 fail closed；所有 destructive path 仍需共用單一 permit/journal，且 shared-directory deletion 與 submodule write 仍待處理。 |
...
的另一個 live instance。在 vendored `agentic-git` 中，
[issue #34](https://github.com/suzuke/agentic-git/issues/34) 也仍存在，因為
`submodule` write 未被分類為 mutating。

**剩餘 invariant。** 把所有 mutation root 與 leaf 遷移到相同 typed
permit/capability 與 durable journal，包括 janitor/retention/GC 和 branch
creation；先修正 shared live-owner deletion 與 upstream submodule
classification，再更新 gitlink。
...
| [agentic-git #34](https://github.com/suzuke/agentic-git/issues/34) | 已確認的 current safety gap | 10, 13 | Mutating-command classification 中缺少 `submodule` 與 `submodule--helper`。 |
