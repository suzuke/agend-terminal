...
的 notification/GC/context 複合 handler）。supervisor `run_loop` 如今只負責
`tick()` / `process_error_recovery()` / 一個 boot-time sidecar GC
（仍是它自己的 10s 執行緒）。
...
（`mcp/handlers/dispatch.rs`）來 dispatch。壓力是微觀層面的：兩個 handler 檔案
...
2. **Heuristic vs hook / observed state，雙 reader**（§2.4）：snapshot 與若干
   ...
   SRL 臂刻意 **KEEP-RAW**（`core.state.current`），避免過期的 Active observed
   遮住真卡住的 agent。同一 tick 內兩套真相仍可能並存；範圍由 escalation
   throttle 限制。進一步 promote 見 #2466 / REFACTOR W3.3——**不是**把每個
