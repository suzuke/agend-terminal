...
| 快捷鍵 | 說明 |
|--------|------|
| `Ctrl+B D` | 開啟決策面板；可檢視記錄並回答 pending question |
| `Ctrl+B T` | 開啟任務看板（五欄看板視圖） |
| `Ctrl+B ?` | 開啟快捷鍵說明 |
...
`backend` 預設為 `claude`。支援的 preset 包括 claude、kiro-cli、codex、opencode、antigravity-cli（`agy`）與 grok。
...
- **Tasks**：五欄看板（Todo / Working / Review / Blocked / Done）
- **Fleet**：agent 列表與狀態
...
TUI 會自動儲存當前的佈局狀態到 `$AGEND_HOME/session.json`，包含：
