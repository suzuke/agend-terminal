...
AgEnD 整合六種 AI coding backend（Claude Code、Kiro CLI、Codex、OpenCode、
Antigravity CLI 與 Grok Build），每個 backend 有不同的安裝路徑、CLI 參數和環境變數。新使用者面對的第一個
問題是：「我裝了什麼？我要怎麼把它跑起來？」
...
成功偵測群組後，quickstart 會將 `AGEND_TELEGRAM_BOT_TOKEN` 寫入 `$AGEND_HOME/.env`（通常是 `~/.agend/.env`）。寫入時會遷移既有 legacy `AGEND_BOT_TOKEN`。
...
`user_allowlist` 欄位永遠會產生。群組偵測若只看到一位 sender，quickstart 會自動填入其 user ID；否則產生空的 TODO 列表。空列表會 fail-closed，必須先完成才能操作 channel。
...
| Agy | `agy` | （無） | `--continue` |
| Grok Build | `grok` | `--always-approve` | cwd 有 session 時使用 `--continue` |
...
| .env | `$AGEND_HOME/.env` | Bot token 環境變數 |

`$AGEND_HOME` 預設為 `~/.agend`；既有 legacy `~/.agend-terminal` 安裝仍會被偵測。

### Token 安全

- Token 以環境變數名稱（`AGEND_TELEGRAM_BOT_TOKEN`）儲存在 fleet.yaml 中，
  而非明文 token 值
- 實際 token 值只存在 `$AGEND_HOME/.env` 檔案中
- `$AGEND_HOME/.env` 權限為 `0600`（Unix）
