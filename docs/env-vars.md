...
| Name | Purpose | Default (unset) | Valid values / format | Source | Notes |
|------|---------|-----------------|-----------------------|--------|-------|
| `AGEND_HOOK_STATE_POC` | Shadow-mode lifecycle-hook POC gate. When on: (a) the MCP-config writer injects hook state-reporters into the agent's per-workspace `.claude/settings` (scope-respecting; user-global `~/.claude` untouched), and (b) the daemon records the reported hook events and compares them against the screen-heuristic state. | Off (no reporters injected; no events recorded; zero behavior change). | Value-based: exactly `"1"` enables; else off. | `src/mcp_config.rs:193` (injects reporters); `src/daemon/hook_shadow.rs:116` (shadow consumer) | Internal POC, default-OFF. **SHADOW-MODE ONLY** — recorded events do NOT drive state transitions. Empirically verified non-disruptive (2026-06-11 live spawn); the settings upsert preserves pre-existing keys. |
