...
| `AGENTIC_GIT_HOME` | The agentic-git shim's own home (primary name; `AGEND_HOME` is its legacy fallback): the directory whose `bin/` the shim excludes from the PATH it hands the real git, and where it writes its audit lines. Daemon pins it to the daemon home at agent spawn (#1504 family). | Shim: unset → falls back to `AGEND_HOME`. Daemon: injected whenever a home is known. | Absolute directory path, equal to the injected `AGEND_HOME`. | `src/agent/mod.rs:936` (`build_command`, beside `AGEND_HOME`); read in `vendor/agentic-git/crates/agentic-git/src/lib.rs:36` | ⚠️ In `SENSITIVE_ENV_KEYS`: a fleet.yaml / template env cannot redirect it. Without the pin, tests that scope `AGEND_HOME` (`ScopedAgendHome`) and spawn `git` make the shim resolve to itself and trip its recursion guard (~40 deterministic failures in agent shells; CI has no shim on PATH). |
