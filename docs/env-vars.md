...
These are read by the vendored `agentic-git` shim. The `AGENTIC_GIT_*` names are
primary; the corresponding `AGEND_*` names remain accepted as legacy fallbacks.
The three `AGENTIC_GIT_BYPASS*` controls are layered emergency overrides — all
⚠️ security-sensitive.

| Name | Purpose | Default (unset) | Valid values / format | Source | Notes |
|------|---------|-----------------|-----------------------|--------|-------|
| `AGENTIC_GIT_BYPASS`<br>`AGEND_GIT_BYPASS` (legacy) | Layer-1 one-shot override: if set, `should_bypass()` returns true and execs real git, skipping all enforcement. | Not bypassed. | **Presence-based** (`is_ok()`); any value, even empty. | `vendor/agentic-git/crates/agentic-git/src/lib.rs:541` | ⚠️ Emergency override. Daemon-internal callers set `=1` to skip the shim. The `AGEND_*` spelling is accepted as a legacy fallback. |
| `AGENTIC_GIT_BYPASS_AGENT`<br>`AGEND_GIT_BYPASS_AGENT` (legacy) | Layer-2 agent-specific exemption: bypass when value equals the current agent identity. | No agent exemption. | Agent/instance name string. | `vendor/agentic-git/crates/agentic-git/src/lib.rs:544` | ⚠️ Value-compared against the current agent identity. The `AGEND_*` spelling is accepted as a legacy fallback. |
| `AGENTIC_GIT_BYPASS_UNTIL`<br>`AGEND_GIT_BYPASS_UNTIL` (legacy) | Layer-3 time-limited exemption: bypass while now < the given epoch. | No time exemption. | **Unix epoch seconds** (`u64`, not ISO). | `vendor/agentic-git/crates/agentic-git/src/lib.rs:551` | ⚠️ Expired/unparseable → no bypass. The `AGEND_*` spelling is accepted as a legacy fallback. |
| `AGENTIC_GIT_SHIM_DEPTH`<br>`AGEND_GIT_SHIM_DEPTH` (legacy) | Recursion guard propagated into spawned git; hard-fails at `MAX_SHIM_DEPTH = 3`. | `0`. | Non-negative `u32`; unparseable → 0. | `vendor/agentic-git/crates/agentic-git/src/lib.rs:67` (read); set at `vendor/agentic-git/crates/agentic-git/src/exec.rs:22,53` | Internal sentinel; not normally user-set. Exits 70 when `>= 3` (#1504). The `AGEND_*` spelling is accepted as a legacy fallback. |
| `AGENTIC_GIT_REAL_GIT`<br>`AGEND_REAL_GIT` (legacy) | Escape hatch holding the path to the real git binary so the shim execs git without recursing. Daemon injects it at agent spawn. | Shim: unset → falls back to literal `"git"` then PATH-exclude resolution. Daemon: only injected when not already set. | Absolute path to a git executable; accepted only if it exists. | `vendor/agentic-git/crates/agentic-git/src/exec.rs:87` (read); daemon injects legacy `AGEND_REAL_GIT` at `src/agent/mod.rs:1160-1174` | ⚠️ Correctness-sensitive: wrong/missing value risks recursive-spawn storm (#1504). The `AGEND_*` spelling is accepted as a legacy fallback. |
| `AGENTIC_GIT_HOME`<br>`AGEND_HOME` (legacy) | The agentic-git shim's own home: the directory whose `bin/` the shim excludes from the PATH it hands the real git, and where it writes its audit lines. Daemon pins it to the daemon home at agent spawn (#1504 family). | Shim: unset → falls back to `AGEND_HOME`. Daemon: injected whenever a home is known. | Absolute directory path, equal to the injected `AGEND_HOME`. | `vendor/agentic-git/crates/agentic-git/src/exec.rs:110` (read); daemon injects primary `AGENTIC_GIT_HOME` at `src/agent/mod.rs:941` | ⚠️ In `SENSITIVE_ENV_KEYS`: a fleet.yaml / template env cannot redirect it. Without the pin, tests that scope `AGEND_HOME` (`ScopedAgendHome`) and spawn `git` make the shim resolve to itself and trip its recursion guard (~40 deterministic failures in agent shells; CI has no shim on PATH). |
...
| `AGEND_CI_TRUSTED_HOSTS` | Comma-separated extra hostnames that may receive the CI/forge token (for self-hosted GHE / self-managed GitLab). Security gate: an agent can set `ci_provider_url` via the `ci watch` MCP tool, so the daemon only attaches the token to the built-in SaaS hosts (`api.github.com`, `gitlab.com`, `api.bitbucket.org`), loopback, and the hosts listed here — never to an arbitrary agent-supplied host. | Only the built-in SaaS hosts + loopback are trusted; any other host is polled **unauthenticated**. | `src/daemon/ci_watch/provider.rs` (`host_receives_credentials`) | Set this when watching a self-hosted forge, e.g. `AGEND_CI_TRUSTED_HOSTS=ghe.corp.example,gitlab.internal`. Prevents CI-token exfiltration (the original SSRF was that the token rode requests to any URL). |
